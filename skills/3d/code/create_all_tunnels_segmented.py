#!/usr/bin/env python3
"""
5 Tunnels Final Segmentation Visualization

Creates a 3D visualization showing all 5 tunnels with their final segmentation results
displayed in a single row with different colors for each segment.
...
def load_all_tunnels_segmented():
    """Load final segmentation data for all 5 tunnels"""
    ...
    for tunnel_id in tunnel_ids:
        final_file = f"{base_path}/{tunnel_id}/final.csv"
...
def create_tunnels_visualization(tunnel_data):
    """Create 3D visualization showing all 5 tunnels with segmentation colors"""
    ...
    fig.update_layout(
        title=dict(
            text='Final Segmentation Results of 5 Tunnels',
            x=0.5,
            font=dict(size=24)
        ),
        height=600,
...
def main():
    ...
    print("Creating 3D visualization for all 5 tunnels - Final Segmentation Results...")
    print()
    
    # Load tunnel data
    tunnel_data = load_all_tunnels_segmented()
    ...
    output_file = "report/3d/1_original.html"
...
camera_settings = {
    ...
    for tunnel_id, df in tunnel_data.items():
        ...
        if 'pred' in df.columns:
            print(f"    Segment distribution (pred):")
            for seg in sorted(df['pred'].unique()):
                count = (df['pred'] == seg).sum()
