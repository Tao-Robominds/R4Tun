#!/usr/bin/env python
"""
Reflection ablation orchestrator.

Interleaves LLM inference (Anthropic Claude) for **detecting** and **SAM** only,
then runs those pipeline stages under ``--ablation r``. Stages 1–3 outputs and
parameters are assumed already aligned with m_s_k (copied into reflection).

Prerequisite: ``data/ablation/reflection/{tunnel_id}/`` contains depth map and
upstream artifacts; intrinsic metrics are read from m_s_k outputs by the
reflection analysts.

Usage:
    ./venv/bin/python run_reflection.py 1-1
    ./venv/bin/python run_reflection.py --all
    ./venv/bin/python run_reflection.py 1-1 --dry-run
    ./venv/bin/python run_reflection.py 1-1 --stages detecting
"""

from __future__ import annotations

import argparse
import json
import os
import re
import subprocess
import sys
import time
from pathlib import Path

import anthropic
from dotenv import load_dotenv

load_dotenv()

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

ABLATION_CODE = "r"
ABLATION_FOLDER = "reflection"
CHARACTERISTICS_SUBROOT = "memory+state+knowledge"
DEFAULT_MODEL_TAG = "opus4.6"
CLAUDE_MODEL = "claude-opus-4-6"
MAX_TOKENS = 16384

STAGES = [
    ("detecting", "configurable_detecting.py"),
    ("sam", "configurable_sam.py"),
]

STAGE_TO_PARAM_NAME = {
    "detecting": "detecting",
    "sam": "sam",
}

ANALYST_CLASSES = {
    "detecting": ("detecting.analyst", "ReflectionDetectingAnalyser"),
    "sam": ("segmenting.analyst", "ReflectionSegmentingAnalyser"),
}

PARAM_BASE = Path("configurable/ablation") / ABLATION_FOLDER / "parameters"
AGENTS_DIR = Path("configurable/ablation") / ABLATION_FOLDER / "agents"
PYTHON = sys.executable

STAGE_NAMES = [s[0] for s in STAGES]


# ---------------------------------------------------------------------------
# Env setup
# ---------------------------------------------------------------------------


def _setup_env() -> dict[str, str]:
    env = os.environ.copy()
    env["R4TUN_PIPELINE_OUT_PREFIX"] = f"data/ablation/{ABLATION_FOLDER}"
    env["R4TUN_ABLATION_TUNNEL_SUBROOT"] = CHARACTERISTICS_SUBROOT
    env["PYTHONPATH"] = "."
    return env


# ---------------------------------------------------------------------------
# Prompt building (reflection analysts under reflection/agents/)
# ---------------------------------------------------------------------------


def _import_analyst(stage_name: str):
    module_rel, class_name = ANALYST_CLASSES[stage_name]
    agents_abs = str(AGENTS_DIR.resolve())
    # Detecting analyst prepends m_s_k agents to sys.path; ensure reflection wins for segmenting.
    while agents_abs in sys.path:
        sys.path.remove(agents_abs)
    sys.path.insert(0, agents_abs)

    import importlib

    mod = importlib.import_module(module_rel)
    return getattr(mod, class_name)


def build_prompt(tunnel_id: str, stage_name: str) -> str:
    cls = _import_analyst(stage_name)
    analyser = cls(tunnel_id)
    return analyser.build_llm_prompt_markdown()


# ---------------------------------------------------------------------------
# Claude API
# ---------------------------------------------------------------------------


def call_claude(prompt: str, dry_run: bool = False) -> str:
    if dry_run:
        print("  [dry-run] Skipping API call")
        return ""

    client = anthropic.Anthropic()
    t0 = time.time()
    message = client.messages.create(
        model=CLAUDE_MODEL,
        max_tokens=MAX_TOKENS,
        messages=[{"role": "user", "content": prompt}],
    )
    elapsed = time.time() - t0
    text = message.content[0].text
    tokens_in = message.usage.input_tokens
    tokens_out = message.usage.output_tokens
    print(f"  API call: {elapsed:.1f}s, {tokens_in} in / {tokens_out} out")
    return text


def extract_json_from_response(response_text: str) -> dict:
    pattern = r"```json\s*\n(.*?)\n\s*```"
    matches = re.findall(pattern, response_text, re.DOTALL)
    if not matches:
        raise ValueError("No ```json``` code fence found in Claude response")
    json_str = matches[-1].strip()
    return json.loads(json_str)


# ---------------------------------------------------------------------------
# Parameter save
# ---------------------------------------------------------------------------


def save_parameters(tunnel_id: str, stage_name: str, params: dict, model_tag: str) -> Path:
    param_name = STAGE_TO_PARAM_NAME[stage_name]
    filename = f"parameters_{param_name}_r_{model_tag}.json"
    out_dir = PARAM_BASE / tunnel_id
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / filename
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(params, f, indent=2)
    print(f"  Saved: {out_path}")
    return out_path


# ---------------------------------------------------------------------------
# Subprocess helpers
# ---------------------------------------------------------------------------


def run_pipeline_stage(tunnel_id: str, stage_script: str, model_tag: str, env: dict) -> None:
    cmd = [
        PYTHON,
        f"configurable/{stage_script}",
        tunnel_id,
        "--ablation",
        ABLATION_CODE,
        "--model",
        model_tag,
    ]
    print(f"  Running: {' '.join(cmd)}")
    result = subprocess.run(cmd, env=env, capture_output=True, text=True)
    if result.stdout:
        for line in result.stdout.strip().split("\n"):
            print(f"    {line}")
    if result.returncode != 0:
        print(f"  STDERR: {result.stderr}")
        raise RuntimeError(f"Pipeline stage {stage_script} failed (exit {result.returncode})")


def run_evaluation(tunnel_id: str, env: dict) -> None:
    only_label = Path(f"data/ablation/{ABLATION_FOLDER}/{tunnel_id}/only_label.csv")
    if not only_label.exists():
        print(f"  Skipping evaluation: {only_label} not found")
        return
    cmd = [
        PYTHON,
        "configurable/evaluation.py",
        tunnel_id,
        "--ablation",
        ABLATION_CODE,
        "--schema",
        "auto",
    ]
    print(f"  Running: {' '.join(cmd)}")
    result = subprocess.run(cmd, env=env, capture_output=True, text=True)
    if result.stdout:
        for line in result.stdout.strip().split("\n"):
            print(f"    {line}")
    if result.returncode != 0:
        print(f"  STDERR: {result.stderr}")
        print("  Warning: evaluation failed, continuing")


# ---------------------------------------------------------------------------
# Main loop
# ---------------------------------------------------------------------------


def process_tunnel(
    tunnel_id: str,
    model_tag: str,
    dry_run: bool,
    env: dict,
    stage_filter: list[str] | None = None,
) -> None:
    print(f"\n{'='*60}")
    print(f"TUNNEL: {tunnel_id}  (reflection: detecting + SAM only)")
    if stage_filter:
        print(f"STAGES: {', '.join(stage_filter)}")
    print(f"{'='*60}")

    out_dir = Path(f"data/ablation/{ABLATION_FOLDER}/{tunnel_id}")
    out_dir.mkdir(parents=True, exist_ok=True)
    analysis_dir = out_dir / "analysis"
    analysis_dir.mkdir(parents=True, exist_ok=True)

    for stage_name, stage_script in STAGES:
        if stage_filter and stage_name not in stage_filter:
            continue
        print(f"\n--- Stage: {stage_name} ---")

        print("  Building prompt...")
        prompt = build_prompt(tunnel_id, stage_name)
        print(f"  Prompt length: {len(prompt):,} chars")

        if dry_run:
            prompt_path = analysis_dir / f"reflection_{stage_name}_prompt.md"
            prompt_path.write_text(prompt, encoding="utf-8")
            print(f"  [dry-run] Wrote {prompt_path}")
            call_claude(prompt, dry_run=True)
            continue

        response_text = call_claude(prompt, dry_run=False)

        try:
            params = extract_json_from_response(response_text)
        except (ValueError, json.JSONDecodeError) as e:
            print(f"  ERROR extracting JSON: {e}")
            (analysis_dir / f"{stage_name}_raw_response.md").write_text(
                response_text, encoding="utf-8"
            )
            raise

        save_parameters(tunnel_id, stage_name, params, model_tag)
        run_pipeline_stage(tunnel_id, stage_script, model_tag, env)

    skip_eval = dry_run or (stage_filter and "sam" not in stage_filter)
    if not skip_eval:
        print("\n--- Evaluation ---")
        run_evaluation(tunnel_id, env)

    print(f"\nDone: {tunnel_id}")


def get_all_tunnel_ids() -> list[str]:
    if not PARAM_BASE.exists():
        return []
    return sorted(
        d.name for d in PARAM_BASE.iterdir() if d.is_dir() and not d.name.startswith(".")
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Reflection ablation orchestrator (detecting + SAM + eval)",
    )
    parser.add_argument(
        "tunnel_ids",
        nargs="*",
        help="Tunnel IDs (e.g. 1-1 4-1)",
    )
    parser.add_argument("--all", action="store_true", help="Process all tunnels under reflection parameters")
    parser.add_argument(
        "--model",
        default=DEFAULT_MODEL_TAG,
        help=f"Model tag for parameter filenames (default: {DEFAULT_MODEL_TAG})",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Build prompts only; write to analysis/; skip API and pipeline",
    )
    parser.add_argument(
        "--stages",
        nargs="+",
        choices=STAGE_NAMES,
        help="Run only these stages (e.g. --stages detecting)",
    )
    args = parser.parse_args()

    if args.all:
        tunnel_ids = get_all_tunnel_ids()
        if not tunnel_ids:
            print("No tunnel IDs found under", PARAM_BASE)
            sys.exit(1)
        print(f"Processing {len(tunnel_ids)} tunnels: {', '.join(tunnel_ids)}")
    elif args.tunnel_ids:
        tunnel_ids = args.tunnel_ids
    else:
        parser.error("Provide tunnel IDs or --all")

    env = _setup_env()
    t_start = time.time()

    for tid in tunnel_ids:
        try:
            process_tunnel(tid, args.model, args.dry_run, env, args.stages)
        except Exception as e:
            print(f"\nFAILED on tunnel {tid}: {e}")
            if len(tunnel_ids) == 1:
                sys.exit(1)
            print("Continuing with next tunnel...\n")

    elapsed = time.time() - t_start
    print(f"\nAll done. Total time: {elapsed:.0f}s")


if __name__ == "__main__":
    main()
