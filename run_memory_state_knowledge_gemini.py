#!/usr/bin/env python
"""
Memory+State+Knowledge ablation orchestrator (Google Gemini 3 Flash).
...
Usage:
    ./venv/bin/python run_memory_state_knowledge_gemini.py 1-1
    ./venv/bin/python run_memory_state_knowledge_gemini.py 1-1 3-1-1 4-1
    ./venv/bin/python run_memory_state_knowledge_gemini.py --all
    ./venv/bin/python run_memory_state_knowledge_gemini.py 1-1 --dry-run   # prompts only, no API

**Routine runs** omit ``--model``: default tag ``gemini3flash`` and parameter files ``*_m_s_k_gemini3flash.json``.
"""
...
# Suffix for parameter filenames, e.g. parameters_unfolding_m_s_k_gemini3flash.json
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if code == 429 or status == "RESOURCE_EXHAUSTED" or "quota" in msg or "rate" in msg:
                ...
                print(
                    f"  API call attempt {attempt}/{MAX_RETRIES} failed after {elapsed:.0f}s: "
                    f"rate limit/quota (code={code}, status={status}): {getattr(e, 'message', e)}"
                )
            ...
            if code in (500, 502, 503, 504) or status == "UNAVAILABLE":
                ...
                print(
                    f"  API call attempt {attempt}/{MAX_RETRIES} failed after {elapsed:.0f}s: "
                    f"APIError {code} {status}: {getattr(e, 'message', e)}"
                )
                if attempt == MAX_RETRIES:
                    raise
                wait = 15 * attempt
...
def main():
    parser = argparse.ArgumentParser(
        description="Memory+State+Knowledge ablation orchestrator (Google Gemini 3 Flash)",
    )
    parser.add_argument(
        "tunnel_ids", nargs="*",
