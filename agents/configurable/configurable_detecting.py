...
def load_parameters(tunnel_id):
    ...
    data_param_file = os.path.join('data', tunnel_id, 'parameters', 'parameters_detecting.json')
    if os.path.exists(data_param_file):
        try:
            with open(data_param_file, 'r') as f:
                params = json.load(f)
            print(f"✅ Loaded parameters from data/{tunnel_id}/parameters/parameters_detecting.json")
            return params
        except Exception as e:
            print(f"❌ Error loading parameters: {e}")
            sys.exit(1)
    else:
        print(f"❌ Error: Parameter file not found at data/{tunnel_id}/parameters/parameters_detecting.json")
