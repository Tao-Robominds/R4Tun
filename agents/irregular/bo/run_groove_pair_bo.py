"""
Bayesian Optimization for Groove-Pair K Y Detection Parameters

Optimizes groove-pair detection parameters (4D) + grouped offsets (12D) = 16D total
to minimize mean wrap-aware |dY| between detected K positions and GT K positions.

Line detection parameters are fixed at BO trial 133 values (already well-tuned).
"""
...
class GroovePairObjective:
    """Objective function for groove-pair BO: minimize mean |dY| vs GT K positions."""
    ...
    def __init__(
        ...
        # Load fixed line detection params
        ...
        # Groove-pair params (4D)
        for name, (lo, hi) in search_space['groove_pair_bounds'].items():
            if name == 'k_candidates_per_ring':
        ...
        # Group offsets (12D)
        for name, (lo, hi) in search_space['group_offset_bounds'].items():
            self.dimensions.append(Real(lo, hi, name=name))
            self.param_names.append(name)
        ...
        if verbose:
            print(f"Groove-Pair BO ({len(self.param_names)}D)")
            print(f"  Groove-pair params: 4")
            print(f"  Group offsets: 12")
    ...
    def __call__(self, params: List) -> float:
        """Evaluate groove-pair parameters."""
        self.eval_count += 1
        t0 = time.time()
        ...
        try:
            ...
            # Merge with fixed params (line detection + other fixed)
            full_params = self.fixed_params.copy()
            full_params.update(param_dict)
            full_params['k_detection_method'] = 'groove_pair'
    ...
    def _log(self, params, results, runtime, error=None):
        ...
        trial_id = f"groove_pair_{self.tunnel_id}_{self.eval_count:03d}"
...
def run_groove_pair_bo(
    ...
    """
    config_path = PROJECT_ROOT / "bo" / "complex_staggered" / "configs" / f"groove_pair_{tunnel_id}.json"
    ...
    print(f"\n{'=' * 70}")
    print(f"GROOVE-PAIR DETECTION BO — Tunnel {tunnel_id}")
    print(f"N calls: {n_calls}, N initial: {n_initial}")
    print(f"{'=' * 70}")
    
    objective = GroovePairObjective(config, tunnel_id, data_dir, logs_dir, verbose)
    x0 = [objective.x0]
    y0 = None
    ...
    # Save results
    results_file = os.path.join(logs_dir, f"best_groove_pair_{tunnel_id}.json")
    with open(results_file, "w") as f:
        json.dump({
            "mean_k_y_distance": best_score,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Groove-pair detection BO for 4-1")
