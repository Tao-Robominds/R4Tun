"""
Template-shape geometric segmentation for irregular tunnels (4-1, 5-1).
...
Overlaps resolved by nearest centre. Y-axis wraps (cylindrical). No SAM, no GPU.

Usage:
  # Prove theory with GT centres
  python 3_template_geometric.py 4-1 --segments-file all_segments_gt.csv
  # Production with detected centres
  python 3_template_geometric.py 4-1 --segments-file all_segments.csv
"""
...
# Default template dimensions in pixels (at resolution 0.005: 1 px = 5 mm)
...
def get_trapezoid_vertices_relative(block: str, params: dict) -> np.ndarray:
    ...
    Order: counterclockwise so Path.contains_points works (left-top, left-bottom, right-bottom, right-top).
    """
    if block == "K":
