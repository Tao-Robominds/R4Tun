...
# Repo root: .../agents/ablation/memory+state+knowledge/agents/<this file> → 4 parents up
...
def load_stage_parameters_pretty(
    ...
    """
    Search order:
    1. sam4tun/agents/parameters/{folder}/{tunnel_id}/parameters_*_{model}.json
    2. sam4tun/agents/parameters/sample/{archive_filename}
    """
    tunnel_dir = PARAM_BASE / tunnel_id
    stem = archive_filename.removesuffix(".json")

    for name in (
        f"{stem}_m_s_k_{model_tag}.json",
    ...
    if baseline_path.exists():
        ...
        if tunnel_id.startswith("3-") and archive_filename == "parameters_denoising.json":
            note += " For T3, see T3_CONTINUOUS — do not use sample mask without retention check."
        return text, note

    raise FileNotFoundError(
        f"No parameter file for tunnel {tunnel_id}, stage {archive_filename}. "
        f"Tried {tunnel_dir} and {baseline_path}"
    )
