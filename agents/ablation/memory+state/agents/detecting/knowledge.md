...
### Continuous (`3-*`) — K horizontal alignment

| Topic | Guidance |
|-------|----------|
| Geometry | Continuous joints → **single K Y** across all rings; no ±431 px stagger |
| One-K-knows-all | One anchor → **Y\*** + fixed K shape for every ring; only X varies |
| Detection priority | `horizontal` K seam > `midpoint` > oblique offset; minimize `assume` |
| Anchor rule | `Y* = median(anchor Y)`; pipeline snaps **all** rings to `(X_i, Y*)` — not only `assume` |
| Uniform snap | Post-snap `Y_std < 10 px`, `max |Y − Y*| = 0`; all rows may be `propagated` |
| Param levers | `hough_threshold_horizontal` (35–45 when failure), `maxLineGap_horizontal` (20–30), `minLineLength_horizontal`, `binary_threshold`, `hough_threshold_oblique` (secondary) |
| Anti-patterns | Staggered assume bands (1035–1265 / 1422–1738); leaving anchor rows at per-ring Y while only `assume` is propagated |
| Success metrics | `Y_std < 10 px`, `assume < 10%` pre-snap, K-block IoU **> 0.65** on 3-1-1 |

- **Physical expectation**: K at **approximately the same Y** every ring (no stagger).
- **Evidence**: pipeline often shows 30–50% **assume** when horizontal/oblique detection fails; built-in assume alternates Y like T1/T2.
- **Goal**: maximize real detections at consistent Y; uniform snap all rings to Y*. See **K_UNIFORM_GATE** in cot.
...
| **assume** | Staggered prior-ring copy (wrong on `3-*`) |
| **propagated** | `3-*` only — uniform Y* applied to **all** rings (including former anchors) |
