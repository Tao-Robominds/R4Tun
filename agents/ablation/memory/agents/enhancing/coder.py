...
class EnhancingParameterExtractor:
    def __init__(self, tunnel_id):
        ...
        self.params_dir = Path(f"agents/{tunnel_id}")  # Save under agents/{tunnel_id}/
        self.api_key = "app-AwnQSxSdDfTN7Tez202ZcmxR"
        self.base_url = "https://api.dify.ai/v1"
        
    def load_analysis_data(self):
        """Load the analysis file for this tunnel"""
        analysis_path = self.analysis_dir / "enhancing_analysis.md"
    ...
    def extract_parameters_via_dify(self):
        ...
        if "No enhancing analysis recommendations" in analysis_content:
...
1. **Find these specific parameters in the analysis:**
   - upsampling_stage1_target_distance (target distance for stage 1 upsampling in meters)
   - upsampling_stage2_target_distance (target distance for stage 2 upsampling in meters)
   - upsampling_stage3_target_distance (target distance for stage 3 upsampling in meters)
   - curvature_threshold (curvature threshold for enhancement)
   - depth_threshold_low (low depth threshold in meters)
   - depth_threshold_high (high depth threshold in meters)
   - inter_radius (interpolation radius in meters)
   - duplicate_threshold (duplicate removal threshold in meters)
   - n_segment_start (starting segment number)
   - n_segment_end (ending segment number)
   - num_neighbors (number of neighbors for interpolation)
   - num_interpolations (number of interpolation iterations)
...
```json
{{
  "upsampling_stage1_target_distance": <extracted_value_or_current>,
  "upsampling_stage2_target_distance": <extracted_value_or_current>,
  "upsampling_stage3_target_distance": <extracted_value_or_current>,
  "curvature_threshold": <extracted_value_or_current>,
  "depth_threshold_low": <extracted_value_or_current>,
  "depth_threshold_high": <extracted_value_or_current>,
  "inter_radius": <extracted_value_or_current>,
  "duplicate_threshold": <extracted_value_or_current>,
  "n_segment_start": <extracted_value_or_current>,
  "n_segment_end": <extracted_value_or_current>,
  "num_neighbors": <extracted_value_or_current>,
  "num_interpolations": <extracted_value_or_current>,
  "resolution": <extracted_value_or_current>,
  "window_size": <extracted_value_or_current>
}}
```
...
"""
    ...
    def parse_and_save_parameters(self, api_response):
        ...
        try:
            ...
            param_file = self.params_dir / "parameters_enhancing.json"
    ...
    def run_configurable_script(self):
        ...
        script_path = Path("agents/enhancing.py")
        ...
        try:
            ...
            print(f"✅ Configurable enhancing completed successfully")
            if result.stdout: 
                print(f"Output: {result.stdout}")
            return True, result.stdout
            
        except subprocess.CalledProcessError as e:
            print(f"❌ Configurable enhancing failed")
            if e.stderr: 
    ...
    def process(self):
        ...
        print("🚀 Step 3: Running configurable enhancing script...")
        ...
        print(f"📁 Parameters saved to: agents/{self.tunnel_id}/")
...
def main():
    ...
    extractor = EnhancingParameterExtractor(tunnel_id)
