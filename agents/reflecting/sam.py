...
if len(sys.argv) != 2:
    ...
    print("Example: python 4-2_sam.py 4-1")
    sys.exit(1)
...
# Load parameters with defaults (keeping optimal values from analysis)
segment_per_ring = parameters.get('segment_per_ring', 6)
...
def generate_dynamic_segment_order(segment_per_ring):
    """Generate segment order dynamically like the original version"""
    block_labels = ['K', 'B1']
...
# Project back to point cloud - DYNAMIC block_to_label mapping
block_to_label = {block: i+1 for i, block in enumerate(segment_order)}
