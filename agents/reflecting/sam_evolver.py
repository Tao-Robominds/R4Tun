...
class SAMEvolver:
    ...
    def analyze_point_coverage(self):
        """Analyze current point distribution from final.csv"""
        try:
            ...
            # Analyze point distribution by predicted labels
            point_counts = {}
            point_percentages = {}
            class_names = {
                0: 'Background',
...
"""
        ...
        combined_data = f"""
You are an expert SAM parameter optimization agent for point cloud segmentation.

# SAM Parameter Evolution - Tunnel {self.tunnel_id}
...
**Primary Objective**: Improve {weakest_block} coverage and overall point distribution balance
...
- segment_per_ring: (integer, usually 6 or 7)
- segment_order: (array of strings, e.g., ["K", "B1", "A1", "A2", "A3", "B2"])
...
Provide your analysis first, then ALWAYS conclude with the complete JSON configuration in the exact format above.
    ...
    def replace_sam_parameters(self, recommended_config):
        ...
        try:
            ...
            print(f"✅ Successfully evolved SAM parameters for tunnel {self.tunnel_id}")
    ...
    def get_evolved_parameters(self):
        ...
        response = requests.post(
            ...
            json={
                ...
                "user": f"sam_evolver_{self.tunnel_id}", "files": []
            }
        )
        ...
        os.makedirs(self.data_dir / "analysis", exist_ok=True)
        output_file = self.data_dir / "analysis" / "sam_evolution.md"
        with open(output_file, 'w') as f:
            f.write(f"# SAM Parameter Evolution - {self.tunnel_id}\n\n")
        ...
        if success:
            print(f"🚀 SAM parameters evolved for tunnel {self.tunnel_id}")
        else:
    ...
    def _request_json_only(self):
        ...
        try:
            ...
            json_only_prompt = f"""
Based on the previous analysis for tunnel {self.tunnel_id}, provide ONLY the optimized SAM parameters in valid JSON format.

Current parameters:
{current_params_str}

Requirements:
- Must include: segment_per_ring, segment_order, segment_width
...
"""
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "response_mode": "streaming", "conversation_id": "",
                    "user": f"sam_evolver_json_{self.tunnel_id}", "files": []
                }
            )
            ...
            json_output_file = self.data_dir / "analysis" / f"sam_evolution_json_only_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            with open(json_output_file, 'w') as f:
                f.write(f"JSON-only response for tunnel {self.tunnel_id}:\n\n{result}")
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python sam_evolver.py <tunnel_id>")
        print("Example: python sam_evolver.py 2-2")
        sys.exit(1)
    ...
    if coverage:
        ...
        print(f"📊 Point coverage analysis:")
    ...
    if result:
        print("\n" + "="*70)
        print("SAM PARAMETER EVOLUTION COMPLETED:")
        print("="*70)
