...
def load_parameters(tunnel_id: str, ring_id: int = None, base_dir: str = "data") -> dict:
    """Load detection parameters for physical constants (per-ring → default)."""
    detection_param_root = os.path.join(
        os.path.dirname(__file__), "2_detection", "parameters"
    )
    candidates = []
    if ring_id is not None:
        ring_key = f"r{int(ring_id)}"
        candidates.append(os.path.join(detection_param_root, tunnel_id, ring_key, "parameters_detection.json"))
        candidates.append(os.path.join(base_dir, tunnel_id, ring_key, "parameters_detection.json"))
    candidates.append(os.path.join(detection_param_root, "_default_irregular", "parameters_detection.json"))
...
def evaluate(
    ...
    """Run per-ring evaluation pipeline.

    Args:
        tunnel_id: Tunnel identifier.
        ring_id: Ring identifier (integer).
        base_dir: Base data directory.
        segment_count: Number of segments (auto-detected if None).
    """
    ring_key = f"r{int(ring_id)}"
    unit = f"{tunnel_id}/{ring_key}"
    print("=" * 70)
    print("SEGMENTATION EVALUATION")
    print("=" * 70)
    print(f"Tunnel/ring: {unit}")
    ...
    generate_report(unit, results, class_names, has_gt, output_dir)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Per-ring tunnel segmentation evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python evaluation.py 4-1 116
  python evaluation.py 5-3 190 --data-dir data/ablation --segments 7

Input files:
  - <data_dir>/<tunnel_id>/r<ring_id>/final.csv (with 'segment' for GT)
""",
    )
    parser.add_argument("tunnel_id", help="Tunnel identifier (e.g., 4-1)")
    ...
    evaluate(
        ...
        base_dir=args.data_dir, segment_count=args.segments,
    )
