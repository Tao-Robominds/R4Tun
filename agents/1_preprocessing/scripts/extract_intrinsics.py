"""
Extract Preprocessing Output Intrinsic Metrics for Irregular (Complex Staggered) Tunnels

Adapted from regular pipeline. Thresholds adjusted for larger tunnel diameter (7.5m)
and bigger depth maps (4712x2549 for 5-1, 3459x3261 for 4-1).

Critical Metrics:
    1. pre_theta_coverage_pct  - Is unfolding complete? [98-108%]
    2. pre_depth_map_valid_pixels - Is depth map dense enough? [8k-50k]
    3. pre_point_retention_pct - Is denoising reasonable? [55-98%]
    4. pre_depth_map_max_empty_row_run - No big white areas? [<=150 rows]

Usage:
    python extract_intrinsics.py 4-1 [--data-dir data/wrap] [--output ...]
"""
...
# =============================================================================
# Guardrail Thresholds (adjusted for irregular/complex staggered tunnels)
# Note: 4-1 is a partial scan (~73% coverage), so theta_coverage lower bound
# is very permissive. Both tunnels have larger depth maps (60-83k valid pixels).
# =============================================================================
THETA_COVERAGE_PCT_MIN = 50.0    # Permissive: partial scans (e.g. 4-1=73%) are valid
THETA_COVERAGE_PCT_MAX = 108.0
POINT_RETENTION_PCT_MIN = 55.0   # Relaxed: GT-optimal denoising keeps more BG points
POINT_RETENTION_PCT_MAX = 98.0
DEPTH_MAP_VALID_PIXELS_MIN = 8_000
DEPTH_MAP_VALID_PIXELS_MAX = 100_000  # Larger images (12M pixels) need higher ceiling
DEPTH_MAP_MAX_EMPTY_ROW_RUN = 350    # 4-1 has 301 from partial scan edge effects
...
def _extract_point_retention(tunnel_dir: str) -> Optional[float]:
    ...
    Returns retention as %. Good range: 55-98%.
    """
...
def _extract_depth_map_valid_pixels(tunnel_dir: str) -> Optional[int]:
    ...
    Good range: 8k-50k (for irregular tunnels with larger depth maps).
    """
...
def _extract_depth_map_max_empty_row_run(tunnel_dir: str) -> Optional[int]:
    ...
    Detects large horizontal white bands that break line detection.
    Good range: <=150 rows (proportional to taller depth maps).
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract preprocessing intrinsic metrics for irregular tunnels",
    ...
    out_path = args.output or os.path.join(tunnel_dir, "preprocessing_intrinsics.json")
