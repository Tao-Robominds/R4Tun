...
def load_parameters(
    ...
    """Load parameters_preprocessing.json for one ring.

    Priority:
        1. agents/.../parameters/<tunnel_id>/r<ring_id>/parameters_preprocessing.json
        2. <base_dir>/<tunnel_id>/r<ring_id>/parameters_preprocessing.json
        3. agents/.../parameters/_warm_start/<regime_label>/parameters_preprocessing.json
        4. agents/.../parameters/_default_irregular/parameters_preprocessing.json
...
# Canonical raster for BO-vs-GT detection learning.
RESOLUTION_M = 0.005


def canonical_h_pixels(tunnel_diameter: float, resolution: float = RESOLUTION_M) -> int:
    """Canonical theta-axis pixels for one full tunnel circumference."""
...
def _fit_circle_xz(x: np.ndarray, z: np.ndarray) -> Tuple[float, float, float]:
    """Algebraic least-squares circle fit in the X-Z plane.

    Returns (cx, cz, R). Points are downsampled (>=20k) to keep the solve
    cheap on dense rings while remaining angle-balanced (centroid converges
    to the geometric center as long as the sample covers the ring).
    """
    n = len(x)
    if n > 20000:
...
def unfold_single_ring(
    ...
    """Single-ring unfolding (Y axial; X-Z cross-section).

    Bypasses the multi-ring tunnel-direction estimator and ellipse fitting
    that fail on a single ring (axis ambiguity, sparse slices). Computes
    (r, theta, h) directly using a least-squares circle fit for the ring
    center (median is biased when point density is uneven across segments):

        cx, cz, R = circle-fit(x, z)
        r       = sqrt((x-cx)^2 + (z-cz)^2)
        theta_d = (degrees(arctan2(z-cz, x-cx)) + 90) % 360   # bottom=0, right=90, top=180, left=270
        theta   = theta_d * (pi * D / 360)                    # arc length [m]
        h       = y - y.min()

    Returns (df_out, ring_count=1, n_planes=1) to match unfold_point_cloud.
    """
    x = df['x'].to_numpy(dtype=np.float64)
    y = df['y'].to_numpy(dtype=np.float64)
    z = df['z'].to_numpy(dtype=np.float64)
    cx, cz, R_fit = _fit_circle_xz(x, z)
    print(f"  circle fit: cx={cx:.3f} cz={cz:.3f} R={R_fit:.3f}")
    ...
    df_out = df.copy()
    df_out['r'] = r.astype(np.float32)
    df_out['theta'] = theta.astype(np.float32)
    df_out['h'] = h.astype(np.float32)
    return df_out, 1, 1
...
def generate_depth_map(
    ...
    fixed_x_min: float = None,
    fixed_x_max: float = None,
    ...
    x_min = float(min(surface[0].min(), boundary[0].min())) if fixed_x_min is None else float(fixed_x_min)
    x_max = float(max(surface[0].max(), boundary[0].max())) if fixed_x_max is None else float(fixed_x_max)
    y_min = float(theta_origin)
    if canonical_height is None:
        y_max = float(max(surface[1].max(), boundary[1].max()))
        height = int(round((y_max - y_min) / resolution))
    else:
        height = int(canonical_height)
    width = int(round((x_max - x_min) / resolution)) + 1
...
def enhance_point_cloud(
    ...
    canonical_height = canonical_h_pixels(tunnel_diameter, resolution=depth_map_resolution)
    ...
    np.save(os.path.join(tunnel_dir, "depth_map.npy"), depth_map)
...
def run_preprocessing(
    ...
    """Run preprocessing on a single ring point cloud.

    Inputs:
        {base_dir}/{tunnel_id}/r{ring_id}/{tunnel_id}_r{ring_id}.txt

    Outputs into the same per-ring folder:
        unwrapped.csv, denoised.csv, enhanced.csv, depth_map.npy, depth_map.png,
        depth_map_outlier.npy, pixel_to_point.pkl, ring_count.txt (=1).
    """
    ring_key = f"r{int(ring_id)}"
    print(f"{'=' * 60}")
    print(f"Preprocessing Pipeline: {tunnel_id}/{ring_key}")
    print(f"{'=' * 60}")
    ...
    if not np.isclose(float(depth_map_resolution), RESOLUTION_M):
        print(
            f"  WARNING: depth_map_resolution={depth_map_resolution} requested, "
            f"forcing canonical {RESOLUTION_M} for BO/GT raster compatibility"
        )
    depth_map_resolution = RESOLUTION_M
    ...
    # ---- Stage 1: Unfolding (single ring, Y-axial) ----
    print("\n[Stage 1] Unfolding (single-ring)...")
    filepath = os.path.join(tunnel_dir, f"{tunnel_id}_r{int(ring_id)}.txt")
    df_raw = load_point_cloud(filepath)
    df_unwrapped, ring_count, n_planes = unfold_single_ring(df_raw, tunnel_diameter)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Per-ring preprocessing for irregular tunnels"
    )
    parser.add_argument("tunnel_id", help="Tunnel identifier (e.g., 4-1, 5-3)")
