...
class DetectingAnalyser:
    ...
    def load_analysis_data(self):
        ...
        role_path = Path("ablation/detecting/role.md")
        role_content = ""
        if role_path.exists():
        ...
        # Load instructions
        instructions_path = Path("ablation/detecting/cot.md")
        ...
        new_characteristics_path = Path(f"data/{self.tunnel_id}/characteristics/pointcloud_characteristics.json")
        ...
        code_path = Path("mes/4-1_detection.py")
        code_content = ""
        if code_path.exists():
        ...
        if detecting_params_path.exists():
            with open(detecting_params_path, 'r') as f:
                detecting_parameters = json.dumps(json.load(f), indent=2)
