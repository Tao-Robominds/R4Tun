...
SAM_CHECKPOINT = "p4tun/segment-anything/sam_vit_h_4b8939.pth"
...
# Simple staggered: 6 segments per ring
SIMPLE_STAGGERED_SEGMENTS = 6
BLOCK_LABELS = ['K', 'B1', 'A1', 'A2', 'A3', 'B2']
BLOCK_TO_LABEL = {'K': 1, 'B1': 2, 'A1': 3, 'A2': 4, 'A3': 5, 'B2': 6}


def compute_block_label():
    """Get block labels for simple staggered (6 segments)."""
    return BLOCK_LABELS


def compute_block_to_label_map():
    """Get block name to numeric label mapping for simple staggered."""
    return BLOCK_TO_LABEL
...
def process_row(df_row, image, predictor, config):
    ...
    # Simple staggered: 6 segments
    block_labels = compute_block_label()
    ...
    results = []
    for i in range(SIMPLE_STAGGERED_SEGMENTS):
        if not reverse:
...
def run_sam(tunnel_id: str, base_dir: str = "data"):
    ...
    print(f"  Segments per ring: {SIMPLE_STAGGERED_SEGMENTS} (simple staggered)")
    ...
    block_to_label = compute_block_to_label_map()
