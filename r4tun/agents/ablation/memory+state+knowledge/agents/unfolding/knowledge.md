...
The pipeline parameters in `agents/ablation/{condition}/parameters/<tunnel_id>/parameters_unfolding.json` drive the geometry recovery in stage 1. Use the following definitions and empirically validated ranges when setting up a new tunnel:
...
## Unfolding — Classification Criteria (CoT domain knowledge)

Use these thresholds when classifying the tunnel after anchoring:
...
**Classification logic:** Primary classification based on density changes (nearest neighbor distance); secondary based on geometric differences; multiple classifications possible (e.g., DENSE + LARGE-DIAMETER).

## Unfolding — Diagnostic Rules

**DENSE:** Verify slice overcrowding: current slice thickness leads to excessive points per slice (>50% more than sample). Only reduce delta if clear evidence of bottlenecks or degraded fit. Higher density may improve RANSAC robustness.

**SPARSE:** Verify slice underpopulation: insufficient points per slice (<50% of sample). Only increase delta if clear evidence of poor fitting. Moderate density reduction may still be adequate.

**CHALLENGING:** Assess sparsity, scale, quality. Default RANSAC threshold 0.5 may be inadequate for noisier/sparser data; larger scale may need more robust fitting; check compounding factors.

**VERY-SPARSE:** May need larger delta (slice thickness); RANSAC sample size may reduce due to few points per slice; RANSAC threshold may need substantial increase (1.5) for very noisy conditions; coordinate multiple parameter changes if needed.

**LARGE-DIAMETER:** Check vertical_filter_window covers appropriate tunnel height percentage; check slice_spacing_factor vs tunnel length.

**General:** Density compatibility — slice thickness (2×delta) vs nearest neighbor distance. Geometric coverage — vertical_filter_window ~80–90% of tunnel height. RANSAC robustness vs noise and point distribution.

## Unfolding — Parameter Adaptation Logic

- **delta**: Decrease for DENSE, increase for SPARSE; typical range **0.003–0.008**; VERY-SPARSE up to **0.01**
- **slice_spacing_factor**: Generally stable; adjust only for extreme length differences
- **vertical_filter_window**: Scale with tunnel height for LARGE-DIAMETER; target **80–90%** height coverage
- **ransac_threshold**: Usually **0.5**; increase to **1.5** for CHALLENGING (sparsity + large scale + noise)
- **ransac_probability, ransac_inlier_ratio**: Preserve unless specific robustness issues
- **ransac_sample_size**: Usually **6**; reduce to **5** for VERY-SPARSE with insufficient points per slice
- **polynomial_degree**: Keep **3** unless complexity warrants change
- **num_samples_factor**: Generally stable across tunnel types