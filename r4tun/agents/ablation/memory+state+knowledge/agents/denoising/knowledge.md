...
Parameters in `agents/ablation/{condition}/parameters/<tunnel_id>/parameters_denoising.json` control the cylindrical grid used to identify sparse/noisy points before Algorithm 2 produces `denoised.csv`. Keep the following intent and ranges in mind:
