#!/usr/bin/env python
"""
Memory ablation orchestrator (OpenAI GPT-5.4).
...
Usage:
    ./venv/bin/python run_memory_gpt.py 1-1
    ./venv/bin/python run_memory_gpt.py 1-1 3-1-1 4-1
    ./venv/bin/python run_memory_gpt.py --all
    ./venv/bin/python run_memory_gpt.py 1-1 --dry-run   # prompts only, no API

**Routine runs** omit ``--model``: default tag ``gpt5.4`` and parameter files ``*_m_gpt5.4.json``.
"""
...
# Parameter filename tag -> OpenAI Responses API model id
MODEL_TAG_TO_OPENAI: dict[str, str] = {
    "gpt5.4": "gpt-5.4",
}

# Suffix for parameter filenames, e.g. parameters_unfolding_m_gpt5.4.json
...
def main():
    parser = argparse.ArgumentParser(
        description="Memory ablation orchestrator (OpenAI GPT-5.4)",
    )
    parser.add_argument(
        "tunnel_ids", nargs="*",
