"""
SAM Segmentation Script with Pattern Discovery + Wrap-around Handling
Uses segment positions inferred from pattern discovery (NO ground truth)

This script processes each segment at positions from inferred_from_pattern.csv
which are derived from sparse detections + domain knowledge.
"""
...
if len(sys.argv) != 2:
    print("Usage: python 4-2_sam_pattern.py <tunnel_id>")
    print("Example: python 4-2_sam_pattern.py 5-1")
    sys.exit(1)

tunnel_id = sys.argv[1]
base_dir = f"data/{tunnel_id}/"

# Check if inferred_from_pattern.csv exists (pattern discovery output)
inferred_path = os.path.join(base_dir, "inferred_from_pattern.csv")
if not os.path.exists(inferred_path):
    print(f"ERROR: {inferred_path} not found!")
    print("This script requires inferred_from_pattern.csv from pattern discovery.")
    print("Run pattern_discovery.py first to generate it.")
    sys.exit(1)
...
print(f"=" * 70)
print(f"SAM SEGMENTATION WITH PATTERN DISCOVERY + WRAP-AROUND")
print(f"=" * 70)
print(f"Tunnel: {tunnel_id}")
print(f"Mode: Pattern-inferred positions (NO ground truth)")
print(f"Total segments to process: {len(inferred_df)}")
print(f"Rings: {inferred_df['Ring'].nunique()}")
print(f"=" * 70)
...
def sam_segment_all_segments(segments_df, image, resolution=0.005):
    """Process each segment individually at its inferred position."""
    all_results = []
    segment_width = 1200
    K_height = 1079.92
    AB_height = 3239.77
    ...
    for _, segment_row in tqdm(segments_df.iterrows(), total=len(segments_df), desc="Processing segments"):
...
# Process all segments individually using pattern-inferred positions
print("\nProcessing all segments with pattern-inferred positions...")
results = sam_segment_all_segments(inferred_df, image)
...
print(f"\n" + "=" * 70)
print(f"RESULTS (Pattern Discovery + Wrap-around)")
print(f"=" * 70)
print(f"Results saved to {base_dir}/final.csv and {base_dir}/only_label.csv")
print(f"Run evaluation: python sam4tun/evaluation_4+5.py {tunnel_id}")
print(f"=" * 70)
