...
# Slicing parameters
SLICE_HALF_THICKNESS = 0.005  # Half-thickness of slicing planes (meters)
                               # Thinner = sharper slices but fewer points per slice
                               # Thicker = more points but less precise localization

# Ellipse fitting region
MAX_DISTANCE_FROM_TOP = 4.5  # Maximum distance from tunnel top for filtering (meters)
                              # Controls which portion of cross-section is used for fitting
                              # Smaller = only top arc, Larger = more of the ellipse

# Centerline curve fitting
POLYNOMIAL_DEGREE = 3  # Degree of polynomial for centerline curve
                        # Higher = more flexible curve (risk of overfitting)
                        # Lower = smoother curve (may not capture bends)

# RANSAC ellipse fitting parameters
RANSAC_INLIER_RATIO = 0.75  # Expected proportion of inlier points (0.0-1.0)
                             # Higher = stricter, expects more good points
                             # Lower = more tolerant of outliers

RANSAC_CONFIDENCE = 0.9  # Probability of finding a correct model (0.0-1.0)
                          # Higher = more iterations, better result
                          # Lower = faster but may miss optimal fit

RANSAC_MIN_SAMPLES = 5  # Minimum points to sample per RANSAC iteration
                         # Must be >= 5 for ellipse fitting (OpenCV requirement)
                         # Higher = more stable but slower convergence

RANSAC_INLIER_THRESHOLD = 0.8  # Inlier threshold as multiplier of std deviation
                                # Lower = stricter inlier detection
                                # Higher = more permissive

# Arc length sampling resolution
SAMPLES_PER_RING = 1210  # Samples per ring for arc length calculation (~1mm accuracy)
                          # Higher = more precise but slower
                          # Lower = faster but less accurate coordinates

# --- Performance Parameters (tune for your hardware) ---
BATCH_SIZE = 1_000_000  # Batch size for parallel coordinate transformation
                         # Larger = faster but more memory usage

NUM_JOBS = 12  # Number of parallel jobs for coordinate transformation
...
def load_point_cloud(filepath: str) -> pd.DataFrame:
    ...
    Expected format: x, y, z, intensity, segment, ring (6 columns)
    
    Args:
        filepath: Path to the point cloud text file.
        
    Returns:
        DataFrame with columns: x, y, z, intensity, segment, ring
    """
...
def compute_optimal_ring_count(distance: float, ring_spacing: float = RING_SPACING) -> int:
    """
...
def generate_slicing_planes(
    ...
    delta: float = SLICE_HALF_THICKNESS
...
def filter_upper_tunnel_points(
    ...
    max_distance: float = MAX_DISTANCE_FROM_TOP
) -> np.ndarray:
    """
...
class EllipseRANSAC:
    ...
    def __init__(
        self,
        points: np.ndarray,
        inlier_ratio: float = RANSAC_INLIER_RATIO,
        confidence: float = RANSAC_CONFIDENCE,
        min_samples: int = RANSAC_MIN_SAMPLES,
...
def transform_to_cylindrical(
    ...
    z_params: np.ndarray,
    ring_count: int,
    diameter: float = TUNNEL_DIAMETER
) -> np.ndarray:
    """
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python 1_unfolding_clean.py <tunnel_id>")
        print("Example: python 1_unfolding_clean.py 1-4")
        sys.exit(1)
