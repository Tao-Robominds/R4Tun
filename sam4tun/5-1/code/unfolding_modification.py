...
# Set tunnel ID according to input
base_dir = f"data/"
tunnel_id = "5-1"

# Load point cloud data
point_cloud_data = np.loadtxt(os.path.join(base_dir, f"{tunnel_id}.txt"))
...
def generate_slicing_planes_point_cloud(center1, center2, points_xyz, delta):
    ...
    # Use 1.0 m slice spacing instead of 1.2 m
    n = round(l / 1.0)
    min_diff = abs(l - 1.0 * n)
    optimal_n = n

    for candidate_n in [n - 1, n + 1]:
        diff = abs(l - 1.0 * candidate_n)
        if diff < min_diff:
    ...
    for i in range(optimal_n):
        ...
        origin.append(np.append(point_on_plane, 0))
    ...
    slicing_cloud = []
    Delta = delta
    points_xyz_arr = np.asarray(points_xyz)

    for plane in tqdm(planes, desc="Processing planes"):
        a, b, c, d = plane
        Wr = a * points_xyz_arr[:, 0] + b * points_xyz_arr[:, 1] + c * points_xyz_arr[:, 2] + d - Delta
        Wl = a * points_xyz_arr[:, 0] + b * points_xyz_arr[:, 1] + c * points_xyz_arr[:, 2] + d + Delta
        mask = (Wr * Wl <= 0)
        slicing_cloud.append(points_xyz_arr[mask])

    return origin, planes, slicing_cloud

# Increase slice thickness to match new average point spacing
delta = 0.01
...
# ## 3. Ellipse centre fitting of Cloud_Slices
...
for points in point2ds:
    ...
    # Expand the vertical filter window to ±6 m
    filtered_points = [point for point in points if abs(point[1] - y_max) <= 6.0]
...
for i in range(len(slicing_cloud)):
    points_data = np.reshape(filtered_point2ds[i], (-1, 2))
    # Relax the inlier threshold to 1.5 m
    ransac = RANSAC(data=points_data, threshold=1.5, P=0.9, S=0.75, N=5)
...
# ## 4. 3D Curve centre fitting
t = np.arange(ring_count)

# Lower polynomial degree to prevent overfitting
degree = 2
