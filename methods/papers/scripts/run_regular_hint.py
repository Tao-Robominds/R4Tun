...
ablation folder, runs m+s+k inference per regular tunnel, snapshots results
to logs/{tunnel}/regular_hint/{model}/, then restores everything.

Usage:
    python3 methods/papers/scripts/run_regular_hint.py --dry-run
    python3 methods/papers/scripts/run_regular_hint.py --tunnel 2-1 --model opus4.6
    python3 methods/papers/scripts/run_regular_hint.py --skip-existing
"""
...
TS = os.environ.get("REGULAR_HINT_TS") or datetime.now().strftime("%Y%m%d_%H%M%S")
...
def has_hint_run(tunnel: str, model: str) -> bool:
    """Skip only successful runs with a measured hint mIoU."""
    d = hint_dir(tunnel, model)
    return extract_miou(d) is not None


@contextmanager
def swapped_detecting_docs():
    """Backup live detecting docs, install agents_regular versions, restore on exit."""
    backup_dir = REPO_ROOT / "logs" / f"regular_hint_doc_backup_{TS}"
...
def run_combo(tunnel: str, model: str, dry_run: bool) -> dict:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            # Still snapshot params if any were written
            if param_dir.exists():
        ...
        row["hint_miou"] = extract_miou(std_data)
        row["status"] = "ok"
        print(f"  hint mIoU: {row['hint_miou']}")
...
def append_summary(csv_path: Path, row: dict) -> None:
    fields = ["tunnel", "model", "baseline_miou", "hint_miou", "status"]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Regular-tunnel K-pattern hint experiment")
    ...
    model = args.model
    summary_csv = REPO_ROOT / "logs" / f"regular_hint_{TS}_summary.csv"

    queue = []
    for tunnel in tunnels:
        if tunnel not in REGULAR_TUNNELS:
            print(f"  skip non-regular tunnel: {tunnel}")
            continue
