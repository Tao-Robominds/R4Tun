...
\begin{enumerate}
\item \emph{The ceiling is a property of the fixed open-source SAM4Tun pipeline, not of the adaptation mechanism.} Even the open-sourced expert's tuning reaches mIoU $= 0.645$ on its reference tunnel under our parameterised protocol; after adaptation false positives stay $\leq 6\%$ and detection is recovered, the residual error is dominated by class swaps from SAM4Tun's fixed detect-K-then-fixed-offset labelling rule, not by LLM parameter choices.
\item \emph{The mechanism itself is validated.} It produces statistically significant paired gains (Cohen's $d \approx 1.5$--$1.95$; 95\% CIs exclude zero), reaches the expert ceiling on regular tunnels label-free ($0.29 \rightarrow 0.50$--$0.54 $), and the deterministic non-LLM baseline ($0.20$) shows the gain is not reproducible by rule lookup.
...
\end{commentblock}
\begin{responseblock}
In retrospect, the original manuscript should have included 'external' comparisons. We have now added Section~5.2 (Comparison with State-of-the-Art (SOTA)) and \MSSOTATable, presented as an operating-point comparison rather than an accuracy ranking, since the methods are not evaluated under comparable conditions: supervised models on Seg2Tunnel report 0.833--0.904 but require per-point labels and retraining (in-distribution); expert-tuned SAM4Tun exceeds 0.90 but only on curated, per-case-tuned rings. Under our protocol the same pipeline reaches 0.645 on its reference and 0.15 static; R4Tun reaches 0.32--0.34 label-free, training-free, and expert-tuning-free. 
