...
\textbf{Important update (implementation correction affecting key reported figures).} During this revision, the first author of the original SAM4Tun pipeline (Zehao~Ye) joined this work as a co-author and helped us correct the open-source SAM4Tun implementation in three places. With the corrected implementation, the expert reference anchor rises from mIoU~0.645 to 0.892. This directly addresses Reviewer~2's central concern: under the same $m+s+k$ adaptation, near-reference (regular-staggered) tunnels now reach mIoU above 0.80. This \emph{strengthens rather than changes} the paper's central logic: a strong expert-tuned reference anchor is a prerequisite for the LLM-guided adaptation mechanism, and given such an anchor the mechanism lifts near-reference performance to expert level. The author list and CRediT contributions have been updated accordingly. The full 30-tunnel ablation is being refreshed on the corrected pipeline.
...
\end{commentblock}
\begin{responseblock}
We agree the previously reported performance was below deployment level, and this revision now locates the cause precisely. Working with the \emph{first author} of SAM4Tun, we found that the low absolute numbers stemmed largely from the \emph{open-source implementation}, not from the architecture or the adaptation mechanism. Three findings underpin our position:
\begin{enumerate}
\item \emph{The earlier ceiling was an implementation limit, now corrected.} Implementation issues (segment-template resizing and tunnel-axis orientation; detailed in our response to Q4) capped even the expert reference at mIoU~0.645. After correction, the same reference tunnel reaches mIoU~0.892 (OA~0.942), which provides the strong anchor that the paper's design requires.
\item \emph{With a strong anchor, the mechanism meets the expert bar near-reference.} Under the corrected anchor and the \emph{same} $m+s+k$ adaptation, representative near-reference (regular-staggered) tunnels reach mIoU~$\approx$0.86. This meets Reviewer~2's stated 0.80 expectation, and it is reached label-free and with no per-tunnel expert re-tuning. The $m\rightarrow(m+s)\rightarrow(m+s+k)$ progression is preserved over the two regular-staggered tunnels (mean mIoU $\approx 0.33\rightarrow0.81\rightarrow0.86$).
\item \emph{Scope remains constrained and unchanged.} Off-reference/complex tunnels are still limited (a single-anchor limitation we keep as future work), so R4Tun continues to be positioned as an assisted, human-verified adaptation mechanism, not an autonomous final-inspection system.
\end{enumerate}
...
\end{commentblock}
\begin{responseblock}
The OA~$\sim$53\% and mIoU~$\sim$33\% figures were reported under the \emph{previous} open-source
SAM4Tun implementation and were bounded by two correctable issues in that code (Q4). After
correcting the implementation with a SAM4Tun author:
\begin{itemize}[nosep]
  \item the reference anchor reaches OA 0.942 / mIoU 0.892;
  \item near-reference (regular) tunnels reach OA $\approx$0.91--0.94 / mIoU $\approx$0.82--0.89,
  so ``nearly half misclassified'' no longer holds near the reference;
  \item averaged across the five tunnel families, OA rises to $\approx$0.67 and mIoU to $\approx$0.50.
\end{itemize}
The residual (off-reference) error is dominated by class swaps from the fixed labelling template,
not noise or missing masks; this is why mIoU trails OA (each swap lowers two class IoUs at once).
The full decomposition is given in Q2 and Q4.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
We have removed all celebratory relative-gain language. The Abstract, Discussion, and Conclusion now lead with absolute mIoU and OA. Our evidence no longer rests on recovery from a collapsed baseline. With the corrected reference anchor, near-reference absolute performance is high in its own right (reference mIoU~0.892; near-reference mean mIoU~$\approx$0.86 over the two regular-staggered tunnels), and the paired per-tunnel improvements use absolute differences (not ratios) with large effect sizes. We retain the 0.15 static figure only as the intended stress test (one fixed reference applied to 30 diverse tunnels), and present it as evidence for the adaptation mechanism, not for deployment readiness.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
The corrected pipeline now passes it. In the submitted version, the open-source SAM4Tun implementation contained fixable issues (see Q4) that capped even the expert reference at mIoU~0.645; the LLM's 0.50--0.54 on regular tunnels was therefore bounded by the implementation, not by LLM reasoning. Working with the first author of SAM4Tun, we corrected these issues: the reference anchor rises to mIoU~0.892, and under the \emph{same} $m+s+k$ adaptation, the two near-reference (regular-staggered) tunnels reach mean mIoU~$\approx$0.86, i.e.\ above the reviewer's 0.80 expectation, reached label-free and without per-tunnel tuning. 
\end{responseblock}
...
Under our single-fixed-reference protocol, the corrected pipeline reaches mIoU~0.892 on its reference tunnel. The only directly comparable, label-, training-, and tuning-free pair remains R4Tun versus static SAM4Tun (0.15), which R4Tun improves. With the corrected reference anchor, R4Tun also reaches expert-level performance on near-reference (regular-staggered) tunnels.
...
\textbf{R4Tun (this study)} & -- & -- & -- & \checkmark~(auto) & \textbf{$\approx$0.5} \\
\bottomrule
\end{tabular*}
\endgroup
\end{table}
...
\end{commentblock}
\begin{responseblock}
We thank the reviewer for this comment, which prompted us to test the hypothesis directly. The reviewer suggested the limit might be fundamental to SAM4Tun's 3D-to-2D projection and SAM-prompting; our investigation, carried out with the first author of SAM4Tun, shows it is instead an \emph{implementation} limit. Correcting the open-source code in three places raised the reference anchor from mIoU~0.645 to 0.892 and raised near-reference (regular-staggered) tunnels above 0.80 (mean mIoU~$\approx$0.86 under the same $m+s+k$ adaptation):
\begin{enumerate}
\item \emph{Segment-template resizing.} The segmenting stage prompts SAM with a low-resolution template mask (\texttt{mask\_input}) built from a crop of each expected segment. The original code resized these crops with \texttt{ResizeLongestSide(256)}, which distorts non-square crops. This distortion is particularly severe for the tall, narrow key~(K) segment. As a result, SAM received an inconsistent prompt and returned unstable masks. Replacing this with an aspect-preserving \texttt{cv2.resize} on the longest side plus zero-padding gives SAM a faithful, predictable prompt, improving mask quality for non-square segments.
\item \emph{Tunnel-axis orientation (``centre swap'').} The unfolding stage builds the panoramic representation from the fitted cross-section axes. Swapping the two centre axes aligns the unfolding direction with the shield's forward direction and the physical ring order; the ring ordering and handedness of the unfolded image then match the ground truth, so the fixed detect-K-then-walk labelling assigns segments in the correct direction. Without it, the walk is mirrored, producing exactly the class-swap/rotation errors documented in Section~4.6. This is the single largest correction for the near-reference class swaps. \emph{Importantly, the correct axis orientation was identified by an expert after inspecting the final segmentation output. The current agent has no access to the final output during adaptation and therefore cannot make this choice automatically; we have added this to future work as reflective agentic behaviour that can test alternative orientations and converge on the correct one under an appropriate proxy signal, without relying on manual inspection of the final segmentation.}
\item \emph{Evaluation alignment.} We additionally aligned the evaluation with the original SAM4Tun notebook protocol (ring-instance mAP), reported alongside the semantic metrics. This is a reporting-faithfulness alignment for comparability with the original SAM4Tun paper, not a performance change; the semantic mIoU used for all ablation comparisons is unchanged, so cross-condition comparisons remain consistent.
\end{enumerate}
The low mIoU is therefore a property of the open-source \emph{implementation}, not of the architecture or of LLM parameter selection. The adaptation is thus not ``rearranging deck chairs'': with a correctly implemented, strong reference anchor the same mechanism reaches expert-level near-reference accuracy.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
Our evidence-based justification is given in full above. In summary, the low absolute metrics reflected a ceiling in the open-source SAM4Tun implementation and the fixed single-anchor labelling rule. With the corrected implementation, the reference anchor reaches mIoU~0.892, and the adaptation mechanism reaches expert-level near-reference performance label-free. To support this we have:
