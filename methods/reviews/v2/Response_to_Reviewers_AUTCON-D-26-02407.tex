...
\begin{itemize}[nosep]
  ...
  \item adds an error-analysis subsection (Section~4.6; Figs.~11--13; Tables~9--10) decomposing false negatives, false positives, and class swaps, and quantifying four structural limits of the fixed SAM4Tun labelling substrate;
...
\end{commentblock}
\begin{responseblock}
We thank the reviewer for this positive assessment. We have added the four suggested references to Related work and Discussion:
\begin{itemize}[nosep]
  \item Yue et al.\ (AEI 2026; DOI 10.1016/j.aei.2026.104774) on non-contact infrastructure surface inspection;
  \item Yue et al.\ (AUTCON 2026; DOI 10.1016/j.autcon.2026.106769) on AI-assisted damage segmentation in heritage buildings;
  \item Yue et al.\ (TUST 2024; DOI 10.1016/j.tust.2024.105998) on shield-tunnel lining damage mechanisms;
  \item Yue et al.\ (Measurement 2024; DOI 10.1016/j.measurement.2023.113903) on GPR-based tunnel-lining defect recognition.
\end{itemize}
...
\end{commentblock}
\begin{responseblock}
We added a repeatability analysis in Appendix~I (LLM inference repeatability; Table~25) and a brief discussion in Section~4.3. Under $m+s+k$ with temperature set to~0, we compared run~1 and run~2 parameter JSONs on held-out complex-tunnel pairs. On the first nine completed tunnel--LLM pairs (tunnels 4-4, 5-3, and 5-4, three LLMs each):
\begin{itemize}[nosep]
  \item median critical-parameter identity across the 18 shared parameters was 100\% (mean 88\%);
  \item mean $|\Delta\text{mIoU}|$ between the two runs was 0.029 (median 0.000).
\end{itemize}
The full protocol targets 90 pairs (30 tunnels $\times$ 3 LLMs); aggregate statistics will be updated when the remaining pairs complete. Large mIoU swings on isolated pairs coincide with lower parameter identity and reflect failure-recovery reruns rather than benign stochasticity at temperature~0.
\end{responseblock}
...
\end{itemize}
The downstream segmentation backbone may change provided these conditions are met for the new pipeline stages.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
We thank the reviewer for this positive assessment and for highlighting the concept of our approach. We have clarified the focus of our study within the Abstract, Introduction, and Conclusion of the revised manuscript.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
We agree that the absolute metrics are below deployment thresholds for autonomous inspection. We have therefore reframed the study as an exploration of a controlled, cross-LLM, zero-label, no-retraining, no-expert-tuned self-adaptation \emph{mechanism}, rather than a performance-competitive segmentation method. Operating in a single forward pass ($\sim$331~s to $\sim$542~s per tunnel), R4Tun introduces explicit, auditable LLM reasoning into an expert-designed pipeline and serves as a methodological foundation for future development.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
We agree that these absolute figures are low. Section~4.6 (Error analysis) now decomposes where the errors arise. On regular tunnels, adaptation largely removes false negatives (21\% $\rightarrow$ 2\% of ground-truth points) but class swaps persist (26\% $\rightarrow$ 21\%); on complex tunnels, adaptation recovers under-segmented blocks (66\% $\rightarrow$ 17\% false negatives) but recovered points are often mislabelled (class swaps rise to 34\%). False positives remain small ($\leq$6\%) throughout. The residual error is therefore dominated by the fixed positional-labelling rule (K-anchor placement and segment-offset template), not by LLM parameter selection.
\end{responseblock}
...
\end{itemize}
We do \emph{not} claim fitness for structural health monitoring, deformation analysis, water-leakage detection, or Scan-to-BIM without human verification.
\end{responseblock}
...
\begin{itemize}[nosep]
  \item mechanism error maps on representative regular (tunnel 1-1) and complex (tunnel 4-1) examples, Opus-4.6 \mbox{$m+s+k$} only (Figs.~11--12);
  \item a quantitative error-composition table over all ground-truth points (Table~9);
  \item quantified diagnostics for four structural limits of the fixed SAM4Tun labelling substrate (Table~10; Fig.~13): ring-level non-uniform point density, a moving K-anchor, fixed segment-offset template drift, and hard-coded walk direction.
\end{itemize}
The Discussion (Section~5.1) links these failure modes to the stated absolute performance ceiling and to the mechanism-level interpretation of the adaptation results. False positives (background classified as blocks) remain rare ($\leq$6\%); the dominant residual errors are class swaps from the positional labelling rule, not 2D-to-3D reprojection artefacts or widespread noise misclassification as key blocks.
\end{responseblock}
...
\begin{itemize}[nosep]
  ...
  \item geometric feature engineering: cross-section fitting error $\sim$1\,mm RMSE reported, but per-segment mIoU not directly comparable;
...
\end{enumerate}
The low absolute mIoU is therefore primarily a limitation of this labelling substrate, not incorrect LLM parameter selection. Bounded adaptation recovers detection (false-negative reduction) but cannot fix mirror flips or 7-segment template mismatch without rewriting the labelling stage---which lies outside the scope of bounded-parameter adaptation studied here. These constraints define the configuration challenge that future dynamic, context-aware labelling must address; they do not invalidate the mechanism-level finding that structured context enables cross-LLM parameter adaptation within fixed bounds.
\end{responseblock}
...
\end{commentblock}
\begin{responseblock}
We agree that the system is not fit for autonomous real-world tunnel inspection at current accuracy. To address this concern, we have revised the manuscript to frame the contribution honestly and to supply the evidence requested above. Summary of actions:
\begin{itemize}[nosep]
  ...
  \item \textbf{In-depth error analysis:} Section~4.6 (Figs.~11--13; Tables~9--10) decomposes errors and quantifies four structural bottlenecks of the fixed labelling substrate.
