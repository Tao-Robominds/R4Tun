...
\hl{Removing these constraints is future work rather than a parameter change, because each is built into the detect-K-then-fixed-template labelling logic of SAM4Tun. This should not be read as evidence that the SAM4Tun approach is invalid. SAM4Tun was designed as an expert-tuned, deterministic, and auditable pipeline before an LLM adaptation layer was available to infer tunnel-specific context, so fixing the K anchor, segment-offset template, and walk direction was a practical way to make the method reproducible and effective around its calibrated reference configuration. R4Tun changes the design space: once structured state and knowledge can guide bounded updates, the same SAM4Tun substrate can be extended with density-adaptive preprocessing, per-ring K re-anchoring, a variable-length segment template, and handedness inference to form dynamic, context-aware labelling. In the current open-source implementation this requires re-writing the labelling stage, which lies outside the bounded-parameter adaptation studied here; encoding multiple expert reference configurations (Section~\ref{sec:limitations}) would also raise the ceiling. These directions therefore extend, rather than invalidate, SAM4Tun and R4Tun: the present study establishes that structured context lets an LLM adapt parameters within fixed bounds, and isolates exactly which structural parts of the substrate must change next to lift the absolute ceiling.}
```
