# T1-4 (test set): OA ≈ 97.5 F1 ≈ 89.8 mIoU ≈ 83.6
# T2-2 (test set): OA ≈ 98.1 F1 ≈ 91.4 mIoU ≈ 85.7
# T3 (long tunnel, 119 rings): OA ≈ 96.2 F1 ≈ 87.3 mIoU ≈ 80.1
...
if len(sys.argv) != 2:
    print("Usage: python evaluation.py <tunnel_id>")
    print("Example: python evaluation.py 4-1")
    sys.exit(1)
...
def evaluate_csv_data():
    ...
    # NEW: Filter out points beyond class 6 to avoid extra classes
    original_size = len(gt_labels)
    valid_mask = (gt_labels <= 6) & (pred_labels <= 6)
    
    # Log what classes are being filtered out
    gt_beyond_6 = gt_labels[gt_labels > 6]
    pred_beyond_6 = pred_labels[pred_labels > 6]
    if len(gt_beyond_6) > 0 or len(pred_beyond_6) > 0:
        print(f"Filtering out classes beyond 6:")
        for cls in np.unique(np.concatenate([gt_beyond_6, pred_beyond_6])):
    ...
    # Define class names based on the updated classification system
    class_names = {
        0: 'Background',
        1: 'K-block',
        2: 'B1-block',
