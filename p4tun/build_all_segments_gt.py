...
Requires: data/<tunnel_id>/unwrapped.csv (h, theta, segment, ring),
          ...
          p4tun/parameters/<tunnel_id>/parameters_enhancing.json (resolution, optional).

Usage:
  python p4tun/build_all_segments_gt.py 4-1 [--data-dir data] [--output all_segments_gt.csv]
"""
...
# 7-seg complex staggered: segment 1=K, 2=B1, 3=B2, 4=A1, 5=A2, 6=A3, 7=A4
...
def load_parameters_enhancing(tunnel_id: str, base_dir: str) -> dict:
    """Load parameters_enhancing.json for depth_map resolution."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    for path in [
        os.path.join(script_dir, "parameters", tunnel_id, "parameters_enhancing.json"),
        os.path.join(base_dir, tunnel_id, "parameters_enhancing.json"),
        os.path.join(script_dir, "parameters_enhancing.json"),
    ]:
...
def load_depth_map_grid(tunnel_dir: str, tunnel_id: str = None, base_dir: str = None) -> tuple:
    ...
    Else infer from pixel_to_point + enhanced (known (pixel, h, theta) pairs).
    Last resort: denoised extent (may place some blocks outside the image).
    ...
    if tunnel_id and base_dir:
        ...
        resolution = float(
            params_enh.get("depth_map", {}).get("resolution", DEFAULT_DEPTH_MAP_RESOLUTION)
        )

    # Fallback: fit unwrapped extent into depth map so all blocks land inside the map (detectable)
    ...
    # Fallback: infer from pixel_to_point + enhanced
    ...
    # Last resort: denoised (pred!=0) extent
...
def build_all_segments_gt(tunnel_id: str, data_dir: str = "data", out_path: str = None) -> pd.DataFrame:
    ...
    # 1) GT centroids from unwrapped.csv (pipeline's own (h, theta) coordinate system)
    ...
    # 2) Depth map grid: use depth_map_grid.json if present (exact build-time grid), else infer
    x_min, y_min, resolution = load_depth_map_grid(tunnel_dir, tunnel_id=tunnel_id, base_dir=base_dir)

    # 3) Unique rings -> 0-based (e.g. 119..125 -> 0..6)
    ...
    for (seg_num, ring_raw), grp in df_gt.groupby(["segment", "ring"]):
        ...
        # True pixel position: same formula as generate_depth_map (grid_x, grid_y).
        # Do not clamp so each block keeps its distinct X,Y (e.g. A1, A2, A3 have different Y).
...
def main():
    parser = argparse.ArgumentParser(
        description="Build all_segments_gt.csv from ground truth data/<tunnel_id>.txt"
    )
    ...
    build_all_segments_gt(args.tunnel_id, data_dir=args.data_dir, out_path=args.output)
