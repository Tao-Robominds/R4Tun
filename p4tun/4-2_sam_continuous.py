"""
Algorithm 4-2 - SAM-based Tunnel Segment Segmentation (Continuous Pattern)

This version is optimized for continuous patterns (T3-like) with wraparound support.
Automatically detects pattern type from pattern_type.json and enables wraparound
for continuous patterns to handle A2-block boundary issues.

All prompt points, template masks, and processing parameters are configurable
via JSON for Bayesian Optimization tuning.

Pipeline:
    3_enhancing.py → pattern_type.json (pattern classification)
    4-1_detection.py → detected.csv (K positions)
    4-2_sam_continuous.py → final.csv (segmented point cloud)
"""
...
def process_row(df_row, image, predictor, config):
    ...
    # Boundary threshold: distance from edge where wraparound might be needed
    # Use AB_height as threshold since A2 is typically one AB_height below K
    boundary_threshold = convert_to_pixel_coords(AB_height * 0.5, resolution)
    ...
    for i in range(segment_per_ring):
        if not reverse:
            ...
            # Selectively enable wraparound for ANY segment near boundaries
            # Detection heuristic (NO ground truth needed):
            # Check geometrically if crop would cross boundaries or segment is near edges
            # This works for ANY segment name (A1, A2, A3, B1, B2, etc.) - not hardcoded to A2
            crop_y1 = map_y - delta_y
            crop_y2 = map_y + delta_y
            crop_crosses_top = crop_y1 < 0
            crop_crosses_bottom = crop_y2 > img_height
            # Also check if near boundaries (within one AB_height of edge)
            near_top = map_y < boundary_threshold
            near_bottom = map_y > (img_height - boundary_threshold)
            
            # Enable wraparound if crop crosses or is near boundary (works for any segment)
            needs_wraparound = crop_crosses_top or crop_crosses_bottom or near_top or near_bottom
            segment_wraparound = enable_y_wraparound if needs_wraparound else False

            cropped_image, template_mask_logit, prompt_centre, is_wrapped_y = crop_image_and_mask_logits(
                image, initial_x, map_y, 2 * delta_x, 2 * delta_y, block, resolution, template_params, mask_eps, segment_wraparound)
        ...
        if reverse:
            ...
            # Apply same selective wraparound logic for reverse direction
            # Check geometrically for ANY segment near boundaries (not hardcoded to A2)
            crop_y1 = map_y - delta_y
            crop_y2 = map_y + delta_y
            crop_crosses_top = crop_y1 < 0
            crop_crosses_bottom = crop_y2 > img_height
...
def run_sam(tunnel_id: str, base_dir: str = "data", segment_count: int = None):
    ...
    # Check pattern type (for logging/info, but don't auto-enable wraparound)
    # NOTE: Wraparound currently degrades overall performance, so we keep it disabled
    # Pattern classification is still useful for detection strategy and other stages
    pattern_type_path = os.path.join(tunnel_dir, "pattern_type.json")
    if os.path.exists(pattern_type_path):
        with open(pattern_type_path, 'r') as f:
            pattern_info = json.load(f)
        pattern_type = pattern_info.get('pattern_type', 'unknown')
        print(f"Pattern type: {pattern_type} - Wraparound disabled (degrades overall performance)")
        print(f"  Pattern classification still used for detection strategy and other stages")
    else:
        print("pattern_type.json not found - using parameter settings")
