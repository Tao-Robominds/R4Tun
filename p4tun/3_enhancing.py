...
def enhance_from_df(
    ...
    # Save grid so (h, theta) -> pixel (X, Y) can be reproduced exactly for GT centroids
    x_min = min(surface_data['x'].min(), boundary_data['x'].min())
    x_max = max(surface_data['x'].max(), boundary_data['x'].max())
    y_min = min(surface_data['y'].min(), boundary_data['y'].min())
    y_max = max(surface_data['y'].max(), boundary_data['y'].max())
    grid_info = {
        'x_min': float(x_min), 'y_min': float(y_min),
        'x_max': float(x_max), 'y_max': float(y_max),
        'resolution': depth_map_resolution,
        'height': int(depth_map.shape[0]), 'width': int(depth_map.shape[1]),
    }
    with open(os.path.join(tunnel_dir, "depth_map_grid.json"), 'w') as f:
