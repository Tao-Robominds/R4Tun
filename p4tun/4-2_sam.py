...
def run_sam(tunnel_id: str, base_dir: str = "data", segment_count: int = None):
    ...
    # Load SAM model (try project root and p4tun-relative paths)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)  # p4tun -> project root
    candidates = [
    ...
    if sam_checkpoint is None:
        raise FileNotFoundError(
            "SAM checkpoint not found. Download sam_vit_h_4b8939.pth from "
            "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_h_4b8939.pth "
            "and place it in p4tun/segment-anything/ or sam4tun/segment-anything/"
        )
    model_type = "vit_h"
    device = "cuda"
