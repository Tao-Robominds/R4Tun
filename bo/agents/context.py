"""
Helpers for **BO (critical-parameters-only) ablation analyst** prompts.

Identical contract to memory+state+knowledge context but paths resolve under bo/.
Loads raw + stage characteristics, reference parameters, strict output instructions.
"""
...
def pipeline_tunnel_data_dir(tunnel_id: str) -> Path:
    pfx = (
        ...
        or f"data/{ABLATION_FOLDER}"
    ).strip().rstrip("/") or f"data/{ABLATION_FOLDER}"
...
def load_stage_parameters_pretty(
    ...
    suffixed = tunnel_dir / f"{stem}_bo_{model_tag}.json"
    if suffixed.exists():
