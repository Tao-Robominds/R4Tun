...
RUN_ROOT = REPO_ROOT / "logs" / "v5_t123_depth_contract_v1"
...
GATE_RINGS = ["1-2/r59", "2-2/r141", "3-1-1/r28"]
...
def load_t123_panel() -> pd.DataFrame:
...
def stage_ring(ring_key: str, *, clean: bool = True) -> Path:
    ...
    if not src.exists():
        raise FileNotFoundError(f"Missing source ring directory: {src}")
...
def apply_r4tun_depth_contract(ring_path: Path, *, interpolation_window: int = 9) -> dict[str, Any]:
    ...
    # The current ring-native enhancing code gates high-density interpolation
    # on these r4tun-derived n_segment keys, not only the outlier_* aliases.
...
def assert_all_depth_maps_pass(audit: pd.DataFrame, *, label: str) -> None:
    failed = audit[~audit["depth_gate_pass"].astype(bool)].copy()
    if not failed.empty:
        failed_keys = ", ".join(failed["ring_key"].astype(str).tolist())
        raise RuntimeError(f"{label} depth gate failed for: {failed_keys}")
