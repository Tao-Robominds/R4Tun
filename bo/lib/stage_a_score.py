...
def score_candidate_row(
    ...
    *,
    ...
    bonus = anchor_plausibility_bonus(
        rho_k=rho_k,
        rho_ab=rho_ab,
        det_guardrail_passed=bool(row.get("det_guardrail_passed")),
    ...
    *,
    ...
    if int(best["candidate_id"]) == 0:
        ...
        abstained = bool(
            ...
            or not bool(best.get("det_guardrail_passed", False))
        )
    if abstained:
...
def default_models() -> dict[str, dict[str, Any]]:
    return {"p11": load_p11_model(), "a3_slim": load_a3_slim_model()}
