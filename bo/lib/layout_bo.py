...
def _coerce_search_x(ctx: RingContext, x: np.ndarray) -> np.ndarray:
    """Accept v1 (no r) and legacy (offsets+r only) vectors for read/resume."""
    x = np.asarray(x, dtype=float).ravel()
    expected = ctx.search_dim
...
def _resolve_prior_root(ctx: RingContext) -> Path | None:
    ...
    if env:
        return Path(env)
    default = REPO_ROOT / "logs" / "sam4tun_prior_v1"
...
def run_ring_bo(
    ...
    *,
    ...
    print(f"== experience BO ({n_evals} evals, anchor={warm_anchor}, direction_select per trial): {ctx.case_id} ==")
    bo_result = run_gp_bo(
        ctx,
        n_evals=n_evals,
        seed=seed,
