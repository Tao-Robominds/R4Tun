...
All pipeline stage scripts import from here.
"""
...
# ---------------------------------------------------------------------------
# Argument parsing (shared by all pipeline stage scripts)
# ---------------------------------------------------------------------------

def parse_pipeline_args(stage_name: str) -> tuple[str, str, str]:
    """
    Parse CLI for a pipeline stage script.
...
def _repo_root() -> str:
    """Best-effort repo root: grandparent of bo/pipeline/ (this package directory)."""
...
def resolve_ablation_param_file(
    ...
    sam4tun (shared):  agents/ablation/sam4tun/parameters_{stage}.json
    per-tunnel:        agents/ablation/{folder}/parameters/{tunnel_id}/parameters_{stage}{tag}_{model}.json
    ...
    if "param_root" in cond:
        ...
        param_base = os.path.join(root, "agents", "ablation", cond["folder"])
...
def resolve_tunnel_pointcloud_txt(tunnel_id: str) -> str:
    """
    Path to the 6-column raw cloud (cwd = repo root or ``agents/``).
