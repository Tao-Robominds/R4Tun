...
def get_preprocessing_dimensions(tunnel_id: str, agent_type: str = 'complex_staggered') -> Tuple[List, List[str]]:
    ...
    Args:
        tunnel_id: Tunnel identifier (e.g., '4-1', '5-1')
    ...
    if os.path.exists(chars_file):
        with open(chars_file, 'r') as f:
            chars = json.load(f)
        radius = chars.get('cross_section_radius_m', 3.75)  # fallback to 4-1 default
    else:
        # Fallback if characteristics.json doesn't exist
        radius = 3.75
        print(f"Warning: characteristics.json not found for {tunnel_id}, using default radius=3.75")
...
def load_best_from_logs(logs_dir: str, tunnel_id: str) -> Optional[Tuple[List[float], float]]:
    ...
    # Build param list in dimension order (9D)
