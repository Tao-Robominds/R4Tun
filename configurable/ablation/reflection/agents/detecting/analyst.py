#!/usr/bin/env python
# -*- encoding: utf-8 -*-
"""Reflection detecting analyst: m_s_k outputs + intrinsic metrics → LLM prompt (markdown)."""
...
class ReflectionDetectingAnalyser:
    ...
    def load_analysis_data(self):
        ...
        code_path = _REPO_ROOT / "sam4tun/4-1_detection.py"
        code_content = read_required_text(code_path, "Sample detecting code")
        archive_name = "parameters_detecting.json"
        ...
        return {
            ...
            "intrinsic": intrinsic,
            "archive_filename": "parameters_detecting_r_opus4.6.json",
        }
    ...
    def build_llm_prompt_markdown(self, state_context: str = "") -> str:
        ...
        parts = [
            f"# ROLE\n{ctx['role']}",
            f"# REFLECTION TASK\nAdjust **detecting** parameters only. Upstream stages are fixed to the m_s_k run. "
        ...
        parts += [
            f"# REFERENCE DETECTING PARAMETERS (m_s_k baseline — tune from here)\n{ctx['parameters_source']}\n\n```json\n{ctx['parameters']}\n```",
...
def main():
    ...
    print(ReflectionDetectingAnalyser(tunnel_id).build_llm_prompt_markdown())
