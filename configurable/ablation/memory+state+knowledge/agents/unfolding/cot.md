...
**For complex tunnels (dataset IDs `4-*`, `5-*` — T4/T5, 7.5 m diameter, 1.8 m ring length):**
- **Physical ring length is 1.8 m**, not 1.2 m (regular/continuous T1–T3 use 1.2 m).
- **`slice_spacing_factor` MUST be 1.8** (not 1.2). The unfolding stage chooses the number of slices so that `slice_spacing_factor × n` matches tunnel extent; using **1.2** for complex tunnels inflates `ring_count` (often ~15) vs the labeled subset (~10 rings), misaligning detecting, SAM/geometric segmentation, and evaluation.
- Using **1.8** aligns `ring_count` with the physical ring spacing and the GT ring count in the evaluation subsets.
- **Regular/continuous** tunnels (`1-*`, `2-*`, `3-*`) keep **`slice_spacing_factor` = 1.2**.
...
- **slice_spacing_factor**: **1.2** for regular/continuous (`1-*`, `2-*`, `3-*`). **1.8** for complex (`4-*`, `5-*`) — matches 1.8 m ring length. Do not copy 1.2 from 5.5 m tunnels onto complex datasets.
...
- For complex tunnels, confirm `slice_spacing_factor` is **1.8** so `ring_count` matches downstream detecting and segmentation (wrong count breaks ring columns and mIoU).
