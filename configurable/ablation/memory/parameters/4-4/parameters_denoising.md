# Memory-ablation LLM context — tunnel `4-4`
...
```bash
./venv/bin/python skills/scripts/export_llm_parameter_context.py 4-4
```
...
# TARGET TUNNEL — RAW CHARACTERISTICS (tunnel_id=4-4)
```json
{
  "tunnel_id": "4-4",
  "input_file": "/home/boringtao/Projects/R4Tun/data/subsets/4-4.txt",
  "filtered_note": "Contains only characteristics for x, y, z, intensity columns. Ground truth data (segment_type, ring_number) excluded.",
  "point_cloud_analysis": {
    "basic_statistics": {
      "total_points": 1951401,
      "data_structure": {
        "columns": 4,
        "description": "x, y, z, intensity"
      },
    ...
    "tunnel_geometry": {
      "dimensions": {
        "length_x_axis": 18.067730827982594,
        "width_y_axis": 7.720634860292929,
        "height_z_axis": 7.66894483,
        "units": "meters"
      },
      "estimated_diameter": 7.720634860292929,
      "diameter_estimation": {
        "inner_diameter": 7.720634860292929,
        "outer_diameter": 7.720634860292929,
        "average_diameter": 7.720634860292929,
        "median_diameter": 7.720634860292929,
        "ring_thickness": null,
        "ring_thickness_note": "Not estimated from minimum bounding rectangle; use unfolded/denoised characterisers for r-based ring thickness.",
        "description": "Estimated tunnel diameter based on minimum bounding rectangle width (2D XOY projection). May include surrounding infrastructure.",
        "method": "minimum_bounding_rectangle",
    ...
    "point_density": {
      "mean_nearest_neighbor_distance": 0.00795607875205586,
      "median_nearest_neighbor_distance": 0.006422258354792286,
      "min_nearest_neighbor_distance": 0.0004878000000001492,
      "max_nearest_neighbor_distance": 0.24081382544432522,
      "units": "meters"
    }
  }
}
...
Archived tunnel parameters (same file you will save as `configurable/ablation/memory/parameters/4-4/parameters_denoising.json`).
