# Memory-ablation LLM context — tunnel `5-1`
...
```bash
./venv/bin/python skills/scripts/export_llm_parameter_context.py 5-1
```
...
# TARGET TUNNEL — RAW CHARACTERISTICS (tunnel_id=5-1)
```json
{
  "tunnel_id": "5-1",
  "input_file": "/home/boringtao/Projects/R4Tun/data/subsets/5-1.txt",
  "filtered_note": "Contains only characteristics for x, y, z, intensity columns. Ground truth data (segment_type, ring_number) excluded.",
  "point_cloud_analysis": {
    "basic_statistics": {
      "total_points": 1957427,
      "data_structure": {
        "columns": 4,
        "description": "x, y, z, intensity"
      },
    ...
    "tunnel_geometry": {
      "dimensions": {
        "length_x_axis": 18.037276353903977,
        "width_y_axis": 7.70870170556278,
        "height_z_axis": 7.65087879,
        "units": "meters"
      },
      "estimated_diameter": 7.70870170556278,
      "diameter_estimation": {
        "inner_diameter": 7.70870170556278,
        "outer_diameter": 7.70870170556278,
        "average_diameter": 7.70870170556278,
        "median_diameter": 7.70870170556278,
        "ring_thickness": null,
        "ring_thickness_note": "Not estimated from minimum bounding rectangle; use unfolded/denoised characterisers for r-based ring thickness.",
        "description": "Estimated tunnel diameter based on minimum bounding rectangle width (2D XOY projection). May include surrounding infrastructure.",
        "method": "minimum_bounding_rectangle",
    ...
    "point_density": {
      "mean_nearest_neighbor_distance": 0.007954931395401503,
      "median_nearest_neighbor_distance": 0.006422427146289875,
      "min_nearest_neighbor_distance": 0.0004878000000001492,
      "max_nearest_neighbor_distance": 0.23392398518385774,
      "units": "meters"
    }
  }
}
...
Archived tunnel parameters (same file you will save as `configurable/ablation/memory/parameters/5-1/parameters_denoising.json`).
