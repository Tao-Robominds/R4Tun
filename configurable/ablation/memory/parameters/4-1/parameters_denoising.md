# Memory-ablation LLM context — tunnel `4-1`
...
```bash
./venv/bin/python skills/scripts/export_llm_parameter_context.py 4-1
```
...
# TARGET TUNNEL — RAW CHARACTERISTICS (tunnel_id=4-1)
```json
{
  "tunnel_id": "4-1",
  "input_file": "/home/boringtao/Projects/R4Tun/data/subsets/4-1.txt",
  "filtered_note": "Contains only characteristics for x, y, z, intensity columns. Ground truth data (segment_type, ring_number) excluded.",
  "point_cloud_analysis": {
    "basic_statistics": {
      "total_points": 1872537,
      "data_structure": {
        "columns": 4,
        "description": "x, y, z, intensity"
      },
    ...
    "tunnel_geometry": {
      "dimensions": {
        "length_x_axis": 18.102654980950557,
        "width_y_axis": 7.7366736105716605,
        "height_z_axis": 7.71533215,
        "units": "meters"
      },
      "estimated_diameter": 7.7366736105716605,
      "diameter_estimation": {
        "inner_diameter": 7.7366736105716605,
        "outer_diameter": 7.7366736105716605,
        "average_diameter": 7.7366736105716605,
        "median_diameter": 7.7366736105716605,
        "ring_thickness": null,
        "ring_thickness_note": "Not estimated from minimum bounding rectangle; use unfolded/denoised characterisers for r-based ring thickness.",
        "description": "Estimated tunnel diameter based on minimum bounding rectangle width (2D XOY projection). May include surrounding infrastructure.",
        "method": "minimum_bounding_rectangle",
    ...
    "point_density": {
      "mean_nearest_neighbor_distance": 0.008070240054119831,
      "median_nearest_neighbor_distance": 0.006532541667766271,
      "min_nearest_neighbor_distance": 0.0004878000000001492,
      "max_nearest_neighbor_distance": 0.35235537975041675,
      "units": "meters"
    }
  }
}
...
Archived tunnel parameters (same file you will save as `configurable/ablation/memory/parameters/4-1/parameters_denoising.json`).
