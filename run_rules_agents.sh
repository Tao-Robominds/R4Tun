#!/usr/bin/env bash
# Rules baseline: sam4tun/agents with per-tunnel rule params → data/rules/{tunnel_id}/
#
# Usage:
#   ./run_rules_agents.sh --sanity              # gate: 1-1, 2-1, 3-1-1, 4-1, 5-1
#   ./run_rules_agents.sh --all                 # all 30 subsets
#   ./run_rules_agents.sh --all --skip-existing # skip tunnels with performance.md
#   ./run_rules_agents.sh 4-1 5-1               # explicit tunnel ids
#
# Parameters from agents/ablation/rules/rule_adapt.py →
#   sam4tun/agents/parameters/rules/{tunnel_id}/
...
pipeline_print_runtime() {
    ...
    echo ""
    echo "=========================================="
    echo "Rules agents wall clock: ${sec}s (${m}m ${s}s)"
...
LOG_DIR="logs/rules"
RULE_ADAPT="agents/ablation/rules/rule_adapt.py"
...
generate_rules_params() {
    local ids=("$@")
    echo "Generating rules parameters for: ${ids[*]}"
    "$PY" "$RULE_ADAPT" "${ids[@]}"
}
...
# Rules baseline sanity gate

- **Command:** \`./run_rules_agents.sh --sanity\`
...
echo ""
echo "=========================================="
echo "Rules sam4tun/agents baseline"
