#!/usr/bin/env python
"""
Memory+State ablation orchestrator (Google Gemini 3 Flash).
...
Usage:
    ./venv/bin/python run_memory_state_gemini.py 1-1
    ./venv/bin/python run_memory_state_gemini.py 1-1 3-1-1 4-1
    ./venv/bin/python run_memory_state_gemini.py --all
    ./venv/bin/python run_memory_state_gemini.py 1-1 --dry-run   # prompts only, no API

**Routine runs** omit ``--model``: default tag ``gemini3flash`` and parameter files ``*_m_s_gemini3flash.json``.
"""
...
def process_tunnel(tunnel_id: str, model_tag: str, dry_run: bool, env: dict,
    ...
    for stage_name, stage_script, characteriser in STAGES:
        ...
        param_name = STAGE_TO_PARAM_NAME[stage_name]
        param_path = PARAM_BASE / tunnel_id / f"parameters_{param_name}_m_s_{model_tag}.json"
        old_params = None
...
def main():
    parser = argparse.ArgumentParser(
        description="Memory+State ablation orchestrator (Google Gemini 3 Flash)",
    )
    parser.add_argument(
        "tunnel_ids", nargs="*",
