...
_k_corr_raw = str(params.get("k_pattern_correction", "on")).lower()
k_pattern_correction = _k_corr_raw in ("on", "true", "1", "yes")
k_pattern_outlier_tol_px = float(params.get("k_pattern_outlier_tol_px", 215))

_scripts = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "methods", "papers", "scripts")
if _scripts not in sys.path:
    sys.path.insert(0, _scripts)
from k_consensus_lib import correct_k_pattern_y_positions  # noqa: E402
...
adjusted_points, _k_snapped = correct_k_pattern_y_positions(
    adjusted_points,
    tunnel_id,
    k_pattern_correction=k_pattern_correction,
    k_pattern_outlier_tol_px=k_pattern_outlier_tol_px,
)
if _k_snapped:
    print(f"K-pattern consensus snapped {_k_snapped} row(s) (tol={k_pattern_outlier_tol_px}px)")
