...
class SegmentingParameterExtractor:
    ...
    def load_analysis_data(self):
        ...
        analysis_path = self.analysis_dir / "segmenting_analysis.md"
    ...
    def extract_parameters_via_dify(self):
        ...
        if "No segmenting analysis recommendations" in analysis_content:
...
1. **Find these specific parameters in the analysis:**
   - segment_per_ring (number of segments per ring)
   - segment_width (width of each segment in pixels)
   - K_height (K height parameter for projection)
   - AB_height (AB height parameter for projection)
   - angle (angle parameter for segmentation)
...
```json
{{
  "segment_per_ring": <extracted_value_or_current>,
  "segment_width": <extracted_value_or_current>,
  "K_height": <extracted_value_or_current>,
  "AB_height": <extracted_value_or_current>,
  "angle": <extracted_value_or_current>,
  "resolution": <extracted_value_or_current>,
  "sam_checkpoint": <extracted_value_or_current>,
  "model_type": <extracted_value_or_current>,
  "device": <extracted_value_or_current>
}}
```
...
"""
    ...
    def parse_and_save_parameters(self, api_response):
        ...
        try:
            ...
            param_file = self.params_dir / "parameters_sam.json"
    ...
    def run_configurable_script(self):
        """Run the configurable segmenting script"""
        script_path = Path("agents/sam.py")
        ...
        try:
            ...
            print(f"✅ Configurable segmenting completed successfully")
            if result.stdout: 
                print(f"Output: {result.stdout}")
            return True, result.stdout
            
        except subprocess.CalledProcessError as e:
            print(f"❌ Configurable segmenting failed")
            if e.stderr: 
    ...
    def process(self):
        ...
        print(f"🔄 Processing parameterized segmenting for tunnel {self.tunnel_id}")
        print("="*60)
        ...
        print("🚀 Step 3: Running configurable segmenting script...")
        ...
        print(f"✅ Configurable segmenting completed for tunnel {self.tunnel_id}")
        print(f"📁 Results saved to: data/{self.tunnel_id}/final.csv")
        return True
...
def main():
    ...
    extractor = SegmentingParameterExtractor(tunnel_id)
