"""
Helpers for **memory ablation analyst** prompts (not for ``run_agents.sh`` / pipeline stages).

Loads sample vs tunnel ``raw_characteristics.json``, loads per-stage **reference** ``parameters_*.json``
from ``agents/ablation/memory/parameters/<id>/`` or default ``agents/parameters/sample/``, and builds
...
def pipeline_tunnel_data_dir(tunnel_id: str) -> Path:
    """Same layout as ``pipeline_data`` (memory ablation → data/ablation/memory/<id>)."""
