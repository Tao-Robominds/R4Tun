...
### Regular / staggered (`1-*`, `2-*`) — K-PATTERN PRIOR

**Physical prior (use this to guide parameter choices):**
- K-block Y alternates between **two levels** on **odd vs even rings** (ring 0, 2, 4… vs 1, 3, 5…).
- Expected gap between levels: **≈ K_height/2 ≈ 430 px** at resolution 0.005 m/px (reference levels ~1210 px vs ~1640 px on `2-1`/`2-2`).
- X columns are already correct; **only Y detection quality matters** for downstream SAM labelling.

**What good detection looks like:**
- Per-ring `Type` is predominantly **midpoint** (both oblique families intersect).
- Y values form a **two-level staircase**: rings alternate between a low band and a high band with ~400–450 px separation.
- **Failure signatures**: constant Y across all rings, monotonic Y drift, or >10% **default**/**assume** rows.

**Parameter recipe (staggered only):**
- Keep **angle_range_oblique** tight: **[6, 9]** and **[-9, -6]** — widened ranges add false obliques that break the two-level pattern.
- Prefer **hough_threshold_oblique** in **50–60** (not below 50 unless default+assume ≥30% with evidence).
- **maxLineGap_oblique** may increase to **45–50** px when oblique gaps are visible in the depth map but lines fragment.
- Do **not** rely on **assume** propagation — each ring should be geometrically detected; assume on staggered tunnels often locks the wrong level for an entire half of rings.
- **ring_spacing_constant = 1.2** (fixed for this family).

### Continuous (`3-*`) — K-PATTERN PRIOR

**Physical prior (use this to guide parameter choices):**
- K-block Y is **approximately constant across all rings** — no odd/even alternation.
- Joint geometry is continuous (not staggered); oblique K-joint lines may be weaker than on T1/T2.

**What good detection looks like:**
- All rings share Y within a **narrow band** (typically <50 px spread when detection succeeds).
- Per-ring `Type` is **midpoint** or **horizontal** — not **assume** (assume copies staggered logic from the prior ring and is wrong here).
- **Failure signatures**: Y spread >100 px across rings, or >20% **assume**/**default** rows.

**Parameter recipe (continuous only):**
- The built-in **assume** fallback was designed for staggered tunnels — **do not depend on it**; treat high assume rate as detection failure.
- **GT-free failure signals** (from `detected_characteristics.json` state, not labels):
  - `type_distribution`: `assume`+`default` ≥ 30%, or `midpoint`+`assume`+`default` ≥ 50%
  - `spatial_bounds.y_range` spread > **150 px** (success: < 50 px)
- When those signals fire, apply the **smallest** recovery bundle:
  - Lower **hough_threshold_horizontal** to **40–45** (was 50 default)
  - Lower **hough_threshold_oblique** to **40–48** if oblique fragments are visible in state
  - Raise **maxLineGap_horizontal** to **12–15** px; **maxLineGap_oblique** to **45–50** px if gaps are cited
  - Optionally widen **angle_range_oblique** to **±4–10°** only when state shows near-miss slopes
- Keep **ring_spacing_constant = 1.2** (fixed for this family).
- **Trust hierarchy for Y quality** (design knowledge): `midpoint` > `positive_slope`/`negative_slope`/`horizontal` > `assume`/`default`. Tuning should maximise the top tiers so all rings share one Y band.
...
## Walk direction (continuous tunnels — architectural note)

The SAM stage walks block labels around each ring in a fixed code order. **Per-ring walk direction is not a detecting parameter.** On continuous tunnels (`3-*`), mirrored label order (class swaps with correct K anchor) is a known failure mode that **cannot be fixed by detecting-stage tuning alone**. Focus detecting changes on **K-Y anchor quality**; report in your analysis if state shows good joint types but segmentation would still need direction correction at the SAM stage.
