"""
Wrap-Around Experiment C: Per-Ring Unfolding

For each ring, the depth map strip is rolled so the cut falls in a groove
(gap between blocks). This eliminates wrap-around entirely: every block is
contiguous in its ring's unfolded strip.

Changes vs base 3_sam.py:
  - Per-ring cut positions computed from angular boundaries (groove midpoints)
  - Block Y coords shifted per-ring so no block wraps
  - Aggregation un-shifts Y per-ring to original coordinates
...
def run_sam(tunnel_id: str, base_dir: str = "data"):
    ...
    # --- Approach C: compute per-ring cut positions from angular boundaries ---
    # For each ring, find the angular boundary with the widest gap between blocks.
    # That midpoint becomes the cut: rolling the image/coordinates so the cut is
    # at Y=0 guarantees no block wraps.
    print("\n  Computing per-ring cut positions...")
    gt_ab_path = os.path.join(tunnel_dir, 'gt_angular_boundaries.json')
    if not os.path.exists(gt_ab_path):
        raise FileNotFoundError(
            f"gt_angular_boundaries.json required for Approach C (not found in {tunnel_dir})")
    with open(gt_ab_path) as f:
        ab_data = json.load(f)

    ring_cuts = {}  # {ring_id: cut_y} — the Y position to place at Y=0
    gt_x_bands_c = {}
    angular_slices_c = {}
    for ring_key, ring_data in ab_data.items():
        ring_id = int(ring_key.split('_')[1])
        slices = ring_data.get('slices', {})
        x_band = ring_data.get('x_band')
        if x_band:
            gt_x_bands_c[ring_id] = (x_band[0], x_band[1])

        # Find the widest gap between adjacent block boundaries
        boundaries_list = []
        for block_name, s in slices.items():
            y_s = s['y_start'] if isinstance(s, dict) else s[0]
            y_e = s['y_end'] if isinstance(s, dict) else s[1]
            boundaries_list.append((block_name, y_s, y_e))
        boundaries_list.sort(key=lambda b: b[1] if b[1] <= b[2] else b[1] - img_h)

        # Each boundary between blocks is a groove; find the one with the widest gap
        best_cut = 0
        best_gap = -1
        for i in range(len(boundaries_list)):
            b_name, y_s, y_e = boundaries_list[i]
            prev_name, prev_ys, prev_ye = boundaries_list[i - 1]
            # Gap between previous block's end and this block's start
            if y_s >= prev_ye:
                gap = y_s - prev_ye
            else:
                gap = (img_h - prev_ye) + y_s
            if gap > best_gap:
                best_gap = gap
                # Cut at the midpoint of this groove
                best_cut = int((prev_ye + gap / 2) % img_h)

        ring_cuts[ring_id] = best_cut

        # Shift angular slices by -cut_y so they don't wrap
        shifted_slices = {}
        for block_name, s in slices.items():
            y_s = s['y_start'] if isinstance(s, dict) else s[0]
            y_e = s['y_end'] if isinstance(s, dict) else s[1]
            shifted_s = (y_s - best_cut) % img_h
            shifted_e = (y_e - best_cut) % img_h
            shifted_slices[block_name] = (shifted_s, shifted_e)
        angular_slices_c[ring_id] = shifted_slices

    for r in sorted(ring_cuts.keys()):
        print(f"    Ring {r}: cut_y={ring_cuts[r]}")

    # Shift block centroids in all_segments_df
    all_segments_shifted = all_segments_df.copy()
    for idx, row in all_segments_shifted.iterrows():
        ring_id = row['Ring']
        cut_y = ring_cuts.get(ring_id, 0)
        all_segments_shifted.at[idx, 'Y'] = (row['Y'] - cut_y) % img_h
    ...
    # Per-ring: create rolled image strips and process blocks
    print("\n[Step 3] Running SAM segmentation (per-ring unfolding)...")
    all_results = []
    for ring_id in sorted(ring_cuts.keys()):
        cut_y = ring_cuts[ring_id]
        ring_segments = all_segments_shifted[all_segments_shifted['Ring'] == ring_id]
        if len(ring_segments) == 0:
            continue

        # Roll the full image by -cut_y along Y axis
        rolled_image = np.roll(image, -cut_y, axis=0)

        for _, seg_row in ring_segments.iterrows():
            result = process_segment(seg_row, rolled_image, predictor, config)
    ...
    # Aggregate results
    print("\n[Step 4] Aggregating (per-ring unfolding)...")
    ...
    # Build per-ring shifted angular slice lookups
    ring_y_to_block = {}
    for ring_id, slices in angular_slices_c.items():
    ...
    # Aggregate: apply mask + angular boundaries in shifted space,
    # then un-shift Y to original coordinates
    assigned_count = 0
    ...
    for item in all_results:
        ...
        for mapping in crop_info['mappings']:
            ...
            n_cols = img_x_end - img_x_start

            for row_offset in range(crop_rows):
                shifted_y = start_y + row_offset
                orig_y = (shifted_y + cut_y) % img_h

                block_row = inside_block[row_offset, :]
                if block_row.shape[0] != n_cols:
                    continue

                if y_lookup is not None:
                    if y_lookup[shifted_y] != label_val:
                        continue

                label_map[orig_y, img_x_start:img_x_end][block_row] = label_val
                ring_map[orig_y, img_x_start:img_x_end][block_row] = ring_id
                assigned_count += block_row.sum()

    print(f"  Per-ring unfolding ({mask_source}) assignment: {assigned_count:,} pixels")

    # Ring 4 GT pixel mask override (still needed for interlocked ring)
