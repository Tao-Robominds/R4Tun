"""
Wrap-Around Experiment A: Periodic Y Crop Windows

Extends the base SAM pipeline with Y-axis periodic crop windows.
When a block's crop extends past Y=0 or Y=img_height, rows are grabbed
from the opposite edge (same logic as existing X-wrap).

Changes vs base 3_sam.py:
  - crop_image_and_mask_logits: handles Y wrap-around
  - aggregation: handles Y wrap back-projection
...
def crop_image_and_mask_logits(image, cx, cy, crop_width, crop_height, block, resolution, template_params,
    ...
    Handles BOTH X-axis and Y-axis wrap-around for segments crossing image
    boundaries. Y-wrap: when crop extends past top/bottom, grab rows from
    the opposite edge (periodic boundary).
    """
    ...
    x_wrap = x1 < 0 or x2 > img_width
    y_wrap = y1 < 0 or y2 > img_height

    # --- Build Y slices (handling Y wrap) ---
    y_slices = []  # list of (crop_row_start, crop_row_end, img_row_start, img_row_end)
    if not y_wrap:
        y_slices.append((0, y2 - y1, y1, y2))
    elif y1 < 0:
        bottom_rows = img_height + y1  # rows from bottom of image
        top_rows = y2
        y_slices.append((0, img_height - bottom_rows, bottom_rows, img_height))
        y_slices.append((img_height - bottom_rows, img_height - bottom_rows + top_rows, 0, top_rows))
    else:  # y2 > img_height
        main_rows = img_height - y1
        overflow_rows = y2 - img_height
        y_slices.append((0, main_rows, y1, img_height))
        y_slices.append((main_rows, main_rows + overflow_rows, 0, overflow_rows))

    total_crop_h = sum(s[1] - s[0] for s in y_slices)

    # --- Build X slices (handling X wrap) ---
    x_slices = []  # list of (crop_col_start, crop_col_end, img_col_start, img_col_end)
    if not x_wrap:
        x_slices.append((0, x2 - x1, x1, x2))
    elif x1 < 0:
        right_start = img_width + x1
        right_w = img_width - right_start
        x_slices.append((0, right_w, right_start, img_width))
        x_slices.append((right_w, right_w + x2, 0, x2))
    else:  # x2 > img_width
        main_w = img_width - x1
        overflow_w = x2 - img_width
        x_slices.append((0, main_w, x1, img_width))
        x_slices.append((main_w, main_w + overflow_w, 0, overflow_w))

    total_crop_w = sum(s[1] - s[0] for s in x_slices)

    # --- Assemble cropped image ---
    cropped_image = np.zeros((total_crop_h, total_crop_w, 3), dtype=image.dtype)
    for ys in y_slices:
        cy_s, cy_e, iy_s, iy_e = ys
        for xs in x_slices:
            cx_s, cx_e, ix_s, ix_e = xs
            cropped_image[cy_s:cy_e, cx_s:cx_e] = image[iy_s:iy_e, ix_s:ix_e]

    if cropped_image.shape[0] == 0 or cropped_image.shape[1] == 0:
        return None, None, None, None, None

    # Prompt centre in crop coordinates
    if not y_wrap:
        prompt_centre_y = cy - y1
    elif y1 < 0:
        prompt_centre_y = cy + (img_height - (img_height + y1))
    else:
        prompt_centre_y = cy - y1
    ...
    if not x_wrap:
        ...
        prompt_centre_x = (img_width - (img_width + x1)) + cx
    else:
        prompt_centre_x = cx - x1

    prompt_centre = (prompt_centre_x, prompt_centre_y)

    # Build crop_mappings: list of {crop_xy, img_xy} for back-projection
    crop_mappings = []
    for ys in y_slices:
        cy_s, cy_e, iy_s, iy_e = ys
        for xs in x_slices:
    ...
    crop_info = {
        "y1": y_slices[0][2] if not y_wrap else None,
        "y2": y_slices[-1][3] if not y_wrap else None,
        "y_wrap": y_wrap,
        "x_wrap": x_wrap,
...
def run_sam(tunnel_id: str, base_dir: str = "data"):
    ...
    if use_angular_boundaries:
        ...
        # Mappings now contain both crop_x/img_x AND crop_y/img_y for Y-wrap support
        assigned_count = 0
        ...
        for item in all_results:
            ...
            for mapping in crop_info['mappings']:
                ...
                crop_y_start, crop_y_end = mapping['crop_y']
                img_x_start, img_x_end = mapping['img_x']
                img_y_start, img_y_end = mapping['img_y']

                inside_block = pixel_mask[crop_y_start:crop_y_end, crop_x_start:crop_x_end] > 0

                if y_lookup is not None:
                    y_indices = np.arange(img_y_start, img_y_end)
                ...
                target = label_map[img_y_start:img_y_end, img_x_start:img_x_end]
                if update_mask.shape != target.shape:
                    continue

                label_map[img_y_start:img_y_end, img_x_start:img_x_end][update_mask] = label_val
                ring_map[img_y_start:img_y_end, img_x_start:img_x_end][update_mask] = ring_id
        ...
        # --- Legacy DT competition mode (with Y-wrap support) ---
        ...
        for item in all_results:
            ...
            for mapping in crop_info['mappings']:
                ...
                mask_slice = mask[crop_y_start:crop_y_end, crop_x_start:crop_x_end]
                logits_slice = new_logits[crop_y_start:crop_y_end, crop_x_start:crop_x_end]
                
                current_logits = logits_map[img_y_start:img_y_end, img_x_start:img_x_end]
                ...
                if use_template_fallback and template_mask is not None:
                    tmpl_slice = template_mask[crop_y_start:crop_y_end, crop_x_start:crop_x_end]
                ...
                logits_map[img_y_start:img_y_end, img_x_start:img_x_end][update_mask] = composite_logits[update_mask]
                label_map[img_y_start:img_y_end, img_x_start:img_x_end][update_mask] = block_to_label.get(block, 0)
                ring_map[img_y_start:img_y_end, img_x_start:img_x_end][update_mask] = ring_id
