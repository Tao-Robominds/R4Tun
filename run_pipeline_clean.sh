#!/bin/bash
#
# GT-Free Tunnel Segmentation Pipeline (Clean Version)
#
# Usage:
#   ./run_pipeline_clean.sh <tunnel_id>              # Run full pipeline
#   ./run_pipeline_clean.sh <tunnel_id> --from <stage>  # Start from specific stage
#   ./run_pipeline_clean.sh --all                    # Run on all datasets
#   ./run_pipeline_clean.sh --list                   # List available datasets
#
# Stages:
#   1 = unfolding
#   2 = denoising
...
#
# Examples:
#   ./run_pipeline_clean.sh 1-4                  # Full pipeline on 1-4
#   ./run_pipeline_clean.sh 1-4 --from 4         # Start from detection
#   ./run_pipeline_clean.sh 4-1 --base patterns  # Use data/patterns/4-1
#   ./run_pipeline_clean.sh --all                # All datasets
#
...
print_banner() {
    echo -e "${BLUE}"
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║       GT-FREE TUNNEL SEGMENTATION PIPELINE (CLEAN VERSION)       ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
run_stage() {
    ...
    case $stage in
        1)
            ...
            python3 sam4tun/1_unfolding_clean.py "$tunnel_id"
            ;;
        2)
            echo -e "${YELLOW}[Stage 2/6] Denoising...${NC}"
            python3 sam4tun/2_denoising_clean.py "$tunnel_id"
            ;;
        3)
            echo -e "${YELLOW}[Stage 3/6] Enhancing...${NC}"
            python3 sam4tun/3_enhancing_clean.py "$tunnel_id"
            ;;
        4)
            echo -e "${YELLOW}[Stage 4/6] Detection & Pattern Inference...${NC}"
            python3 sam4tun/4-1_detection_clean.py "$tunnel_id"
            ;;
        5)
            if [ "$SKIP_SAM" = true ]; then
                ...
                python3 sam4tun/4-2_sam_clean.py "$tunnel_id"
            fi
            ;;
        6)
            echo -e "${YELLOW}[Stage 6/6] Evaluation...${NC}"
            python3 sam4tun/evaluation_clean.py "$tunnel_id"
            ;;
    esac
}
